...
static int icn_tpd_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int icn_tpd_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);
static int icn_tpd_remove(struct i2c_client *client);
...
static struct i2c_driver icn_tpd_i2c_driver = {
	.driver = {
		.name = "ICN85XX",
	...
	.probe = icn_tpd_probe,
	.remove = icn_tpd_remove,
	.id_table = icn85xx_tpd_id,
	.detect = icn_tpd_detect,
};
...
static ssize_t icn85xx_show_mode(struct device_driver *drv,char* buf);
static ssize_t icn85xx_store_mode(struct device_driver *drv,const char *buf,size_t count);
...
static DRIVER_ATTR(icn_tp_mode, S_IRUGO | S_IWUSR, icn85xx_show_mode, icn85xx_store_mode);
...
static ssize_t icn85xx_store_mode(struct device_driver *drv,const char *buf,size_t count)
{
	pop_mode = simple_strtoul(buf, NULL, 10);
...
static ssize_t icn85xx_show_process(struct device* cd,struct device_attribute *attr, char* buf)
{
	return sprintf(buf, "icn85xx process\n");
}
...
//	int keyarr[]={KEY_O,KEY_W,KEY_M,KEY_E,KEY_C,KEY_UP,KEY_DOWN,KEY_LEFT,KEY_RIGHT,KEY_U};
    ...
    {
        TPD_DMESG("{Power Off Charging...Skip Probe %s\n");
        return 0;
    }
#endif
...
static int icn_tpd_local_init(void)
{
	TPD_DMESG("ICN85XX I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);

	if(i2c_add_driver(&icn_tpd_i2c_driver) != 0)
	{
...
static int icn_tpd_resume(struct i2c_client *client)
{
	int retval = TPD_OK;
	suspend_status=0;
	printk("TPD wake up\n");
...
static int icn_tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	int retval = TPD_OK;
...
static struct tpd_driver_t icn_tpd_device_driver = {
	.tpd_device_name = "ICN85XX",
	.tpd_local_init = icn_tpd_local_init,
	.suspend = icn_tpd_suspend,
	.resume = icn_tpd_resume,
#ifdef TPD_HAVE_BUTTON
	.tpd_have_button = 1,
#else
	.tpd_have_button = 0,
...
static int __init tpd_driver_init(void) {
	...
	if(tpd_driver_add(&icn_tpd_device_driver) < 0)
...
static void __exit tpd_driver_exit(void) {
	...
	tpd_driver_remove(&icn_tpd_device_driver);
}
