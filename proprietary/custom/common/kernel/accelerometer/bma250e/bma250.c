...
static int BMA250_CheckDeviceID(struct i2c_client *client)
{
	...
	else if (databuf[0]==BMA222E_FIXED_DEVID)
	{
		printk("BMA222E_CheckDeviceID %4xh pass!\n ", databuf[0]);	
	}
	else
	{
		printk("BMA250_CheckDeviceID %d fail!\n ", databuf[0]);
		printk("BMA222E_CheckDeviceID %4xh fail!\n ", databuf[0]);
	}
...
#if 0
static int Chivin_Get_ShowLog_Flag(struct i2c_client *client)
{
    u8 flag;
    u8 reg_addr;
    if (bma250_revesion == BMA250)
    {
		reg_addr = BMA250_RESERVED_REG_1;
    }
    else
    {
		reg_addr = BMA250E_RESERVED_REG_1;
    }
    if(hwmsen_read_block(client, reg_addr, &flag, 1)){	
        return 0;
    }
    printk("%s: flag = 0x%x\n", __FUNCTION__, flag);
    if(flag == 0xA2){
...
static int Chivin_Set_ShowLog_Flag(struct i2c_client *client, int flag)
{
    u8 buf;
    u8 data;
	u8 reg_addr;
    ...
    if(hwmsen_read_block(client, reg_addr, &buf, 1)){ 
        return 1;
    }
    if(data == buf){
        return 0;
    ...
    printk("%s: write flag to eeprom\n", __FUNCTION__);		
    BMA250_SetPowerMode(client, TRUE);
    if(hwmsen_write_block(client, reg_addr, &data, 1)){ 
        return 1;
    }
    buf = 0x01;
    if(hwmsen_write_block(client, BMA250_EEPROM_CTL_REG, &buf, 1)){ 
        return 1;
    }
    buf = 0x03;
    if(hwmsen_write_block(client, BMA250_EEPROM_CTL_REG, &buf, 1)){ 
    ...
    //warit for ready flag
    for(i=0; i<100; i++){
        mdelay(10);
        if(0 == hwmsen_read_block(client, BMA250_EEPROM_CTL_REG, &buf, 1)){
            if(buf&0x04){
                printk("%s: i=%d, write eeprom complete!\n", __FUNCTION__, i);
                return 0;
            }
        }
    }
...
//QCI Josh+++
/*-------------------------------------------------------------------------------------------*/
//read bma250 i2c register
//1. echo r [addr] [0x00] > iicrw
//2. cat iicrw
/*-------------------------------------------------------------------------------------------*/
static ssize_t show_iicrw_value(struct device_driver *ddri, char *buf)
{
	struct i2c_client *client = bma250_i2c_client;
	int err;
	...
	else if(err = i2c_smbus_read_byte_data(client, gaddr))
	{
		GSE_ERR("error: %d\n", err);
	}
...
/*-------------------------------------------------------------------------------------------*/
//QCI Josh+++
//write bma250 i2c register
//echo w [addr] [value] > iicrw
/*-------------------------------------------------------------------------------------------*/
static ssize_t store_iicrw_value(struct device_driver *ddri, char *buf, size_t count)
{
	struct i2c_client *client = bma250_i2c_client;
	int res,data;
	char iicrw;
 
	sscanf(buf, "%s %x %x", &iicrw, &gaddr, &data);
 
	if(iicrw=='w')
	{
		res = i2c_smbus_write_byte_data(client,gaddr,data);
		if(res <= 0)
		{
			return BMA250_ERR_I2C;
		}
...
//QCI Josh+++
/*-------------------------------------------------------------------------------------------*/
static void manual_cali(void)
{
	hwmsen_write_byte(bma250_i2c_client, 0x11, 0x00);mdelay(200);// enable sensor
	hwmsen_write_byte(bma250_i2c_client, 0x36, 0x80);mdelay(200);// clear offset compensation
	hwmsen_write_byte(bma250_i2c_client, 0x0F, 0x03);mdelay(200);// set 2g
	hwmsen_write_byte(bma250_i2c_client, 0x10, 0x0A);mdelay(200);// set 31Hz
	hwmsen_write_byte(bma250_i2c_client, 0x37, 0x40);mdelay(200);// set cal z-axis
	hwmsen_write_byte(bma250_i2c_client, 0x36, 0x20);mdelay(1000);// cal on x-axis
	hwmsen_write_byte(bma250_i2c_client, 0x36, 0x40);mdelay(1000);// cal on y-axis
	hwmsen_write_byte(bma250_i2c_client, 0x36, 0x60);mdelay(1000);// cal on z-axis
	return 0;
}
static int write_cali(u8 flag,u8 xoffset, u8 yoffset, u8 zoffset)
{
	...
	sprintf(buf,"%X,%02X,%02X,%02X",flag,xoffset,yoffset,zoffset);

	pfile = filp_open("/data/misc/acdapi/gsensors.dat", O_CREAT|O_WRONLY, 0);
	if (IS_ERR(pfile)) {
		printk("%s: error occured while opening file\n", __func__);
		return -EIO;
	}
	printk("find file\n");
	old_fs = get_fs();
...
static ssize_t show_qcicali_value(struct device *dev, struct device_attribute *attr, char *buf)
{
	int err;
    u8 sx[3]={0,0,0};

	manual_cali();    //HW calibration
	err =  hwmsen_read_block(bma250_i2c_client,0x38,sx,0x03);
	if(err < 0)
	{
		write_cali(0,0,0,0);
		return sprintf(buf,"read offset fail\n");
	}
	else
	{
		write_cali(1,sx[0],sx[1],sx[2]);
		return sprintf(buf,"offset value X = %02X,Y = %02X,Z = %02X\n",sx[0],sx[1],sx[2]);
	}
}
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(qcicali,    S_IWUSR | S_IRUGO, show_qcicali_value,       NULL);
static DRIVER_ATTR(iicrw,      S_IWUSR | S_IRUGO, show_iicrw_value,         store_iicrw_value);
...
/*----------------------------------------------------------------------------*/
int gsensor_operate_bma250(void* self, uint32_t command, void* buff_in, int size_in,
...
static long bma250_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)       
{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_GET_CALI:
			...
			break;
/*		
		case GSENSOR_IOCTL_READ_LOG_FLAG:
			err = Chivin_Get_ShowLog_Flag(client);
			break;*/

// 		case GSENSOR_IOCTL_SET_LOG_FLAG:
// 			err = Chivin_Set_ShowLog_Flag(client, arg);
// 			break;
		default:
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = bma250_create_attr(&bma250_gsensor_driver.driver);
	if(err)
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = gsensor_operate_bma250;
