...
{
        hwmsen_write_byte(bma2xx_i2c_client, 0x11, 0x00);mdelay(200);// enable sensor
        hwmsen_write_byte(bma2xx_i2c_client, 0x36, 0x80);mdelay(200);// clear offset compensation
        hwmsen_write_byte(bma2xx_i2c_client, 0x0F, 0x03);mdelay(200);// set 2g
        hwmsen_write_byte(bma2xx_i2c_client, 0x10, 0x0A);mdelay(200);// set 31Hz
        hwmsen_write_byte(bma2xx_i2c_client, 0x37, 0x40);mdelay(200);// set cal z-axis
        hwmsen_write_byte(bma2xx_i2c_client, 0x36, 0x20);mdelay(1000);// cal on x-axis
        hwmsen_write_byte(bma2xx_i2c_client, 0x36, 0x40);mdelay(1000);// cal on y-axis
        hwmsen_write_byte(bma2xx_i2c_client, 0x36, 0x60);mdelay(1000);// cal on z-axis
        return 0;
}
...
static ssize_t show_qcicali_value(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	u8 sx[3]={0,0,0};
        ...
        err =  hwmsen_read_block(bma2xx_i2c_client,0x38,sx,0x03);
        if(err < 0)
        {
                write_cali(0,0,0,0);
