...
static enum power_supply_property battery_props[] = {
    ...
    //Barkly, for dump charger IC registers
    POWER_SUPPLY_PROP_ChargerReg,
	//Barkly, sysfs node for AC-in wakelock
	POWER_SUPPLY_PROP_AC_In_wakelock,
	//Barkly, sysfs node for enable/disable charging
	POWER_SUPPLY_PROP_ChargingOff,
};

//Barkly, for dump charger IC registers
#define bq24196_REG_NUM 11
kal_uint8 BQ24196_ChargerReg[bq24196_REG_NUM] = {0};
char szChargerReg[256] = {0};

//Barkly, sysfs node for AC-in wakelock, 1: enable, 0 disable
int AC_In_wakelock = 1;
...
//Barkly, extern func from bq24196.c, for dump charger IC registers
extern int bq24196_read_byte(kal_uint8 cmd, kal_uint8 *returnData);

//Barkly, for dump charger IC registers
void bq24196_read_all_registers(void)
{
    int i=0;
    char szTemp[256];
    ...
    {
        bq24196_read_byte(i, &BQ24196_ChargerReg[i]);
        sprintf(szTemp, "Reg[0x%X]=0x%X\n", i, BQ24196_ChargerReg[i]);
        strcat(szChargerReg, szTemp);
        //printk("[%s] Reg[0x%X]=0x%X\n",__func__, i, BQ24196_ChargerReg[i]);        
    }
}
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_present_2nd :
        ...
        break;
	//Barkly, for dump charger IC registers
    case POWER_SUPPLY_PROP_ChargerReg :
        bq24196_read_all_registers();
        val->strval = szChargerReg;
        break;
	//Barkly, sysfs node for AC-in wakelock
	case POWER_SUPPLY_PROP_AC_In_wakelock:
        val->intval = AC_In_wakelock;
        break;
	//Barkly, sysfs node for enable/disable charging
	case POWER_SUPPLY_PROP_ChargingOff:
        val->intval = ChargingOff;
        break;
    default:
        ret = -EINVAL;
        break;
...
//Barkly, sysfs node for AC-in wakelock(write)
    ...
    union power_supply_propval *val)
{
	...
	//Barkly, sysfs node for AC-in wakelock
	case POWER_SUPPLY_PROP_AC_In_wakelock:
        AC_In_wakelock = val->intval;
        break;
	//Barkly, sysfs node for enable/disable charging
	case POWER_SUPPLY_PROP_ChargingOff:
        ChargingOff = val->intval;
        if(ChargingOff == 1)
        {
...
#ifdef MTK_WIRELESS_CHARGER_SUPPORT //Barkly
...
static struct battery_data battery_main = {
	...
	.set_property = battery_set_property, //Barkly, sysfs node for AC-in wakelock
    },
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	static Counter = 3; //Barkly, -1% each 30 sec, better experience for end user
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
#ifdef MTK_DUAL_INPUT_CHARGER_SUPPORT
	kal_uint32 v_charger_max = DISO_data.vusb_hv_voltage;
#endif
	if( BMT_status.charger_exist == KAL_TRUE)
    {
        ...
        if(DISO_data.diso_state.cur_vdc_state == DISO_ONLINE)
            v_charger_max = DISO_data.vdc_hv_voltage;         
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
		...
		if(BMT_status.temperature >= 55) //Barkly, based on power team spec.
        {
            #if defined(CONFIG_POWER_EXT)
            ...
            #else
            {
                ...
                else
                {
                    ...
                    if( BMT_status.charger_exist == KAL_TRUE )
                    {
                        battery_xlog_printk(BAT_LOG_CRTI, "[Battery] Tbat(%d)>=55, AC-in, system need power down.\n", BMT_status.temperature);
                    ...
                    {
                        battery_xlog_printk(BAT_LOG_CRTI, "[Battery] Tbat(%d)>=60, NO AC, system need power down.\n", BMT_status.temperature);
        ...
        //Barkly, batt_temp <= -5'C, AC-in:reset, No AC:power off
        else if(BMT_status.temperature <= -5)
        {
			if( BMT_status.charger_exist == KAL_TRUE )
			{
				battery_xlog_printk(BAT_LOG_CRTI, "[Battery] Tbat(%d)<=-5, AC-in, system need power reset.\n", BMT_status.temperature);
				// can not power down due to charger exist, so need reset system
                battery_charging_control(CHARGING_CMD_SET_PLATFORM_RESET,NULL);
			}
			battery_xlog_printk(BAT_LOG_CRTI, "[Battery] Tbat(%d)<=-5, No AC, system need power down.\n", BMT_status.temperature);
			//avoid SW no feedback
            battery_charging_control(CHARGING_CMD_SET_POWER_OFF,NULL);
		}
#endif
...
#if 1 //Barkly, make sure charger type detection called not only for MTK_WIRELESS_CHARGER_SUPPORT
...
static void mt_battery_charger_detect_check(void)
{
    if( upmu_is_chr_det() == KAL_TRUE )
    {
        if(AC_In_wakelock) //Barkly, sysfs node for AC-in wakelock
		{
			//printk("[%s] Enable AC_In_wakelock \n",__func__);
			wake_lock(&battery_suspend_lock);
		}
		else
		{
			//printk("[%s] Disable AC_In_wakelock \n",__func__);
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
		...
		battery_charging_control(CHARGING_CMD_GET_DISO_STATE,&diso_state);
		...
		if(((diso_state >> 1) & 0x3) != 0x0)
		{
				switch (diso_state){
						...
						default: // OTG only also can trigger vcdt IRQ
								...
								return; // OTG plugin no need battery sync action
				}
#else
        if( upmu_is_chr_det() == KAL_TRUE )
        {
...
#endif
        battery_xlog_printk(BAT_LOG_CRTI, "[do_chrdet_int_task] battery thread not ready, will do after battery init.\n");    
   	}
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	if(DISO_data.diso_state.cur_vdc_state == DISO_ONLINE)
	{
	    hv_voltage = DISO_data.vdc_hv_voltage;         
	} else {
	    hv_voltage = DISO_data.vusb_hv_voltage;         
	}
#endif
...
void set_vusb_auxadc_irq(bool  enable, bool flag)
{
	...
	if (enable)
	{
		if (flag == 0)
		{
			threshold = DISO_data.vusb_measure_channel.falling_threshold;
		} else {
			threshold = DISO_data.vusb_measure_channel.rising_threshold;
		}
		threshold = (threshold *R_DISO_VBUS_PULL_DOWN)/(R_DISO_VBUS_PULL_DOWN + R_DISO_VBUS_PULL_UP);
		mt_auxadc_enableBackgroundDection(DISO_data.vusb_measure_channel.number, threshold, \
												DISO_data.vusb_measure_channel.period, \
												DISO_data.vusb_measure_channel.debounce, flag);
	} else {
		mt_auxadc_disableBackgroundDection(DISO_data.vusb_measure_channel.number);
	}
}
...
void set_vdc_auxadc_irq(bool enable, bool flag)
{
	...
	if(enable)
	{
		if(flag == 0)
		{
			threshold = DISO_data.vdc_measure_channel.falling_threshold;
		} else {
			threshold = DISO_data.vdc_measure_channel.rising_threshold;
		}
		threshold = (threshold *R_DISO_DC_PULL_DOWN)/(R_DISO_DC_PULL_DOWN + R_DISO_DC_PULL_UP);
		mt_auxadc_enableBackgroundDection(DISO_data.vdc_measure_channel.number, threshold, \
												DISO_data.vdc_measure_channel.period, \
												DISO_data.vdc_measure_channel.debounce, flag);
	} else {
		mt_auxadc_disableBackgroundDection(DISO_data.vdc_measure_channel.number);
	}
}
static irqreturn_t   diso_auxadc_irq_handler()
{
...
static irqreturn_t diso_auxadc_irq_thread()
{
...
static void dual_input_init()
{
	...
	DISO_data.switch_rising_timing  = SWITCH_RISING_TIMING ;
	DISO_data.switch_falling_timing = SWITCH_FALLING_TIMING; 

	DISO_data.irq_line_number = LOWBATTERY_IRQ_ID;

	DISO_data.vdc_hv_voltage  = VDC_MAX_VOLTAGE;         
	DISO_data.vdc_lv_voltage  = VDC_MIN_VOLTAGE;         
	DISO_data.vusb_hv_voltage = VBUS_MAX_VOLTAGE;         
	DISO_data.vusb_lv_voltage = VBUS_MIN_VOLTAGE;         

	DISO_data.vdc_measure_channel.number   = AP_AUXADC_DISO_VDC_CHANNEL;
	DISO_data.vusb_measure_channel.number  = AP_AUXADC_DISO_VUSB_CHANNEL;
	DISO_data.vdc_measure_channel.period   = AUXADC_CHANNEL_DELAY_PERIOD;
	DISO_data.vusb_measure_channel.period  = AUXADC_CHANNEL_DELAY_PERIOD;
	DISO_data.vdc_measure_channel.debounce   = AUXADC_CHANNEL_DEBOUNCE;
	DISO_data.vusb_measure_channel.debounce  = AUXADC_CHANNEL_DEBOUNCE;

	/* use default threshold voltage, if use high voltage,maybe refine*/
	DISO_data.vusb_measure_channel.falling_threshold  = VBUS_MIN_VOLTAGE/1000;
	DISO_data.vdc_measure_channel.falling_threshold   = VDC_MIN_VOLTAGE/1000;
	DISO_data.vusb_measure_channel.rising_threshold  = VBUS_MIN_VOLTAGE/1000;
	DISO_data.vdc_measure_channel.rising_threshold   = VDC_MIN_VOLTAGE/1000;
	mt_irq_set_sens(DISO_data.irq_line_number, MT_EDGE_SENSITIVE);
	mt_irq_set_polarity(DISO_data.irq_line_number, MT_POLARITY_LOW);

	ret = request_threaded_irq(DISO_data.irq_line_number, diso_auxadc_irq_handler, diso_auxadc_irq_thread, IRQF_ONESHOT  , "DISO_ADC_IRQ", NULL);
	if (ret)
	{
		...
		set_vdc_auxadc_irq(DISO_IRQ_DISABLE, 0);
		set_vusb_auxadc_irq(DISO_IRQ_DISABLE, 0);
		DISO_data.irq = LOWBATTERY_IRQ_ID;
	}
#ifdef MTK_DISCRETE_SWITCH
