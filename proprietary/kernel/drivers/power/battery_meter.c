...
#elif defined(SOC_BY_SW_FG) || defined(SOC_BY_HW_FG)
{
	...
	printk("%s _g_bat_sleep_total_time=%d g_spm_timer=%d\n"
	, __func__
	, _g_bat_sleep_total_time
	, g_spm_timer);
	//if (_g_bat_sleep_total_time < g_spm_timer) {
	if(abs(_g_bat_sleep_total_time - g_spm_timer) < 500) { //Barkly, for accurate capacity in suspend
		return 0;
	}
	_g_bat_sleep_total_time = 0;
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &g_hw_ocv_before_sleep);

	printk("%s g_hw_ocv_before_sleep=%d \r\n",__func__, g_hw_ocv_before_sleep);
}	
#endif	
...
static int battery_meter_resume(struct platform_device *dev)
{
    ...
    if(bat_is_charger_exist() == KAL_FALSE)
    {
    	...
    	if(1)	//Barkly, fixes incorrect capacity in suspend mode, do not consider suspend time at all.
    	{
