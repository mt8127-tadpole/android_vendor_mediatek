...
status_t DashDataProcessor::handleFmp4Buffer(  sp<ABuffer> &buffer){
    ...
    for (size_t i = mPacketSources.size(); i-- > 0;) {
        ...
        if(mFormats.indexOfKey(trackIndex) >= 0){
            ...
            }else if(packetSource->getPendingFormat() == NULL) {
                if(packetSource->getFormat().get() != (mFormats.editValueFor(trackIndex)).get()){ //new stream
                    ALOGI("%s [%p] setFormatPending",streamMask2LogTag(),this);
                    packetSource->setPendingFormat(mFormats.editValueFor(trackIndex));
                }             
           }
        }
        ...
        for (size_t i = mPacketSources.size(); i-- > 0;) {
            ...
            if (packetSource->getFormat() == NULL) {
                packetSource->setFormat(source->getFormat());
                 ...
                 && (packetSource->getFormat().get() != source->getFormat().get())) //new stream
                packetSource->setPendingFormat(source->getFormat());
