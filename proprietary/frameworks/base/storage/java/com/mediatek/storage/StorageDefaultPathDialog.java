...
// +++ albert to supress SD card insertion dialog before device provisioned for NAR-1561.
// otherwise, the Settings functions incorrectly.
...
public class StorageDefaultPathDialog extends AlertActivity implements DialogInterface.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		// otherwise, the Settings functions incorrectly.
		try {
			if (1 != Settings.Global.getInt(getContentResolver(), Settings.Global.DEVICE_PROVISIONED))
				finish();
		} catch (Exception ex) { // android.provider.Settings.SettingNotFoundException
			ex.printStackTrace();
		}
		// --- albert
    }
    ...
    private void onOK() {
		...
		// +++ albert to ask for Parent Lock Code in kids mode while inserting micro SD-card for default write disk
        // startActivity(intent);
		Intent intentPLC = new Intent("com.leapfrog.action.VALIDATE_PARENT_PASSWORD");
        intentPLC.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NO_HISTORY);
		intentPLC.putExtra("com.leapfrog.extra.REQUESTOR_FOR_PARENT_PIN", intent);
        startActivity(intentPLC);
		// --- albert
