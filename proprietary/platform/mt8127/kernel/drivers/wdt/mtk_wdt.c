...
static int mtk_wdt_deglitch_disable(void)
{
...
static int mtk_wd_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	//mtk_wdt_ModeSelection(KAL_FALSE, KAL_FALSE, KAL_FALSE);
	// en debug, dis irq, dis ext, low pol, dis wdt
	mtk_wdt_deglitch_disable();
	return 0;
#if 0
...
static int mtk_wd_resume(struct platform_device *pdev)
{
#if 0	
	if ( g_wdt_enable == 1 ) 
	{
...
static struct platform_driver mtk_wdt_driver =
{
	...
	.suspend	= mtk_wd_suspend,
	.resume	= mtk_wd_resume,
};
