...
//    		GPIOMSG("[GPIO] 0 Read data: 0x%x \n", data);
    		if(data < 0) return -ERWRAPPER;
    		data = data & 0xFFFFFFFE;
//    		GPIOMSG("[GPIO] 0 After & data: 0x%x \n", data);
    		GPIOEXT_WR(RG_USBDL_EN_CTL_REG, data);
//    		GPIOMSG("[GPIO] 0 After Write & read data: 0x%x \n", GPIOEXT_RD(RG_USBDL_EN_CTL_REG));
    	} else {
    		data = GPIOEXT_RD(RG_USBDL_EN_CTL_REG);
//    		GPIOMSG("[GPIO] 1 Read data: 0x%x \n", data);
    		if(data < 0) return -ERWRAPPER;
    		data = data | 0x00000001;
//    		GPIOMSG("[GPIO] 1 After & data: 0x%x \n", data);
    		GPIOEXT_WR(RG_USBDL_EN_CTL_REG, data);
//    		GPIOMSG("[GPIO] 1 After Write & read data: 0x%x \n", GPIOEXT_RD(RG_USBDL_EN_CTL_REG));
    	}
    }
