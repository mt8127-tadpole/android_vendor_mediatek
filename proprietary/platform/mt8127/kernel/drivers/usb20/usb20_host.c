...
#define VBUS_DETECT_PIN_GPIO GPIO33
#define VBUS_DETECT_PIN_EINT 44
#endif
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
    ...
    if(is_on){
        ...
        //Barkly 1/22/2015, pull high GPIO_OTG_DRVVBUS_PIN to BQ24196:OTG pin
        ...
        //Barkly 1/22/2015, pull GPIO_OTG_DRVVBUS_PIN low to BQ24196:OTG pin
        mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_OTG_DRVVBUS_PIN_M_GPIO);
    ...
    if(is_on){
        ...
        DBG(0,"GPIO16 pull up to plus vbus\n");
        mt_set_gpio_mode(GPIO16, GPIO_MODE_01);
    ...
    {
        /*close vbus by GPIO16 control VBAT.*/
        DBG(0,"GPIO16 pull down to close vbus\n"); 
        mt_set_gpio_mode(GPIO16, GPIO_MODE_01);
        mt_set_gpio_dir(GPIO16, GPIO_DIR_OUT);
        mt_set_gpio_pull_enable(GPIO16, GPIO_PULL_ENABLE);
        mt_set_gpio_pull_select(GPIO16, GPIO_PULL_DOWN);
        msleep(100);
...
#elif defined(OTG_BOOST_BY_SWITCH_CHARGER) //Barkly 1/22/2015
	mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_OTG_DRVVBUS_PIN_M_GPIO);//should set GPIO2 as gpio mode.
	mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN,GPIO_DIR_OUT);
	mt_get_gpio_pull_enable(GPIO_OTG_DRVVBUS_PIN);
	mt_set_gpio_pull_select(GPIO_OTG_DRVVBUS_PIN,GPIO_PULL_UP);
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	if (mtk_musb->is_host) {
		...
		//Adjust TERM_VREF_EL=460mv, VRT_VREF_SEL=820mv for USB eye pattern testing
