...
static void fill_nand_bmt_buffer(bmt_struct * bmt, u8 * dat, u8 * oob)
{
    ...
    phys_bmt = (phys_bmt_struct *)kmalloc(sizeof(phys_bmt_struct),GFP_KERNEL);

    if(!phys_bmt) 
    {
        printk("[fill_nand_bmt_buffer]kmalloc phys_bmt_struct fail!\n");
        while(1);
    }
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    int i;
    phys_table = (phys_bmt_struct *)kmalloc(sizeof(phys_bmt_struct),GFP_KERNEL);
    
    if(!phys_table) 
    {
        printk("[load_bmt_data]kmalloc phys_bmt_struct fail!\n");
        while(1);
    }
