...
extern void lcd_power_en(unsigned char enabled);
...
#endif
        MMProfileLogEx(MTKFB_MMP_Events.HWCFence[i], MMProfileFlagPulse, 2, (unsigned int)fences[i]);
        /* Unref the fence */
...
int mtkfb_queue_overlay_config(struct mtkfb_device *fbdev, struct fb_overlay_config* config)
{
    ...
    for (i = 0; i < HW_OVERLAY_COUNT; i++) {
        ...
        MMProfileLogEx(MTKFB_MMP_Events.HWCFence[i], MMProfileFlagPulse, layer->fence_fd, (unsigned int)work->fences[i]);
