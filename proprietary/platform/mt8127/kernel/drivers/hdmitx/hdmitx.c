    ...
    for (;;)
    {
        ...
        if (p->is_clock_on == true) ///remove the first head here
        {
            if (!list_empty(&HDMI_Buffer_List))
            {
                ...
                if (pBuffList
                    && (reg_configed == pBuffList->buf_state || reg_updated == pBuffList->buf_state || buf_read_done == pBuffList->buf_state)
                    && flag_resolution_interlace(p->output_video_resolution))
                {
                    if(hdmi_rdma_address_config(true, pBuffList->buffer_info) < 0)
                    {
                       HDMI_LOG(" rdma config(pBuffList %x) error to exit (0)\n", pBuffList);
                       //pBuffList->buf_state = buf_read_done;
                    }
                }
                else if ((pBuffList == NULL) || (pBuffList->buf_state != insert_new))
                {
                    ...
                    /* For interlace output, we need config the current buffer again according TV field. */
                    if (flag_resolution_interlace(p->output_video_resolution))
                    {
                        spin_lock_bh(&hdmi_lock);
                        pBuffList = list_first_entry(&HDMI_Buffer_List, hdmi_video_buffer_list, list);
                        ...
                        while(pBuffList) {
                            ...
                            if (list_is_last(&pBuffList->list, &HDMI_Buffer_List))
                            {
                                pBuffList = NULL; // avoid config rdma address below.
                                break;
                            }
                        ...
                        if (pBuffList)
                        {
                            if(hdmi_rdma_address_config(true, pBuffList->buffer_info) < 0)
                            {
                                HDMI_LOG(" rdma config(pBuffList %x) error to exit (1)\n", pBuffList);
                                //pBuffList->buf_state = buf_read_done;
                            }
                        }
                    }
                ...
                {
                    /* For mtk overlay engine, sync_fenc_wait ok when post_video_buffer, so the buffer can used directly here.*/
                    if(hdmi_rdma_address_config(true, pBuffList->buffer_info) < 0)
                    {
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ...
    if (enable)
    {
        BOOL b_is_top_field = DPI1_IS_TOP_FIELD();
        ...
        if (true == need_config) {
            ...
            hdmi_set_rdma_addr(config.addr, buffer_size, buffer_info.security);   //Really setting frame buffer address
            ...
            if (buffer_info.security != gRDMASecure)
            {
                ...
                hdmi_set_rdma_addr(config.addr, buffer_size, buffer_info.security);
...
#if ((!defined(MTK_MT8193_HDMI_SUPPORT))&&( !defined(MTK_INTERNAL_HDMI_SUPPORT))&&(!defined(MTK_INTERNAL_MHL_SUPPORT)))||(defined(MTK_ALPS_BOX_SUPPORT))
...
extern int disp_rdma1_intr_restore(void);
extern int disp_rdma1_intr_disable_and_clear(void);
...
/*static*/ void hdmi_power_on(void)
{
    ...
    // Add synchronization mechanism with hdmi_rdma_config_kthread to fix the issue:
    // when RDMA1 clock is off, then disp_path_config_ is called in hdmi_rdma_address_config.
    // It will lead to fail to clear RDMA1 intr, the irq handler will occupy the CPU.
    if (wait_event_interruptible_timeout(hdmi_rdma_update_wq, atomic_read(&hdmi_rdma_update_event), HZ / 20) == 0)
    {
        printk("[hdmi] wait rdma wq timeout @%s, %d, early_suspend=%d, %d\n",
            __FUNCTION__, __LINE__, is_early_suspended, p->is_clock_on);
    }
...
/*static*/ void hdmi_power_off(void)
{
    ...
    disp_rdma1_intr_disable_and_clear();
	...
	)
	{
	   ...
	   hdmi_drv->colordeep(hdmi_colorspace);
...

            {
                ...
                printk("[hdmi] set rdma1 target line %d,hdmi_height %d,res %d\n",line,p->hdmi_height, arg);
        ...
        case MTK_HDMI_POST_VIDEO_BUFFER:
        {
            ...
            printk("POST Buffer Info: va:0x%x mva:0x%x fmt:0x%x, pitch:%d, ROI(%d,%d,%d,%d), "
            ...
            if(p->is_clock_on)
            {
                ...
                if (pBuffList->buffer_info.fenceFd >= 0)
                {
                    ...
                    if(sync_fence_wait(pBuffList->fence, 1000) < 0) {
                        printk("[error] rdma1 acquire fence timeout phy=0x%08x\n", pBuffList->buffer_info.src_phy_addr);
                        pBuffList->buf_state = buf_dropped;
                    }
                }
                // for debug -- colorbar
	 ...
	 /*check_res  0:HDMI Plug in; 1 HDMI Plug in,EDID OK; -1: No HDMI Plug in; -2: DPI Clock OFF; -3: HDMI Plug in, EDID Error */
