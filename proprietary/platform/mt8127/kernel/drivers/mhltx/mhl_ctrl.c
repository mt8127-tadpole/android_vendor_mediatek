...
void mt_hdmi_debug_write(char *buf)
{
    ...
    else if (strncmp(buf, "w:",2) == 0)
    {
     ...
     printk(KERN_ERR "w:0x%08x=0x%08x\n", reg, val);
#if (defined(MTK_IN_HOUSE_TEE_SUPPORT) && defined(MTK_HDMI_HDCP_SUPPORT))  
    ...
    else if (strncmp(buf, "r:",2) == 0)
    {
        ...
        vadr_regend  &= 0x3ff;
        printk(KERN_ERR "r:0x%08x/0x%08x\n", vadr_regstart, vadr_regend);
    ...
    else if (strncmp(buf, "r6:",3) == 0)
    {
        ...
        while (vadr_regstart <= vadr_regend)
        {
      	        printk(KERN_ERR "0x%08x = 0x%08x\n",vadr_regstart,u4ReadCbus(vadr_regstart));
		 vadr_regstart += 4;
        }
    }
   ...
   else if (strncmp(buf, "help",4) == 0)
   {
        ...
        printk(KERN_ERR "  if open tz and hdcp config, hdmi reg addr is 0x15000+offset, otherwise addr is real PA\n");
