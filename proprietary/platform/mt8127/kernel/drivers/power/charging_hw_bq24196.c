...
#endif
	   if( upmu_get_rgs_chrdet() == 0 ) //Barkly, fixes USB AC/USB detection error, why MTK patch removed it?
...
static kal_uint32 diso_get_current_voltage(int Channel)
{
        ...
        while (i--)
        {
            ...
            ret += ret_temp;
            battery_xlog_printk(BAT_LOG_FULL, "[DISO] ret_temp=%d\n",ret_temp);
        }
        
        ret = ret*1500/4096 ;
        ret = ret/times;
        battery_xlog_printk(BAT_LOG_CRTI, "[DISO]  auxadc measured input voltage mV = %d\n",ret);
...
static kal_uint32 charging_get_diso_state(void *data)
{
	...
	kal_bool  vin_state = GPIO_OUT_ZERO;
#else
	kal_bool  vin_state = GPIO_OUT_ONE;
#endif
...
#endif
		} while ((!vin_state) && (i*AUXADC_CHANNEL_DELAY_PERIOD < SWITCH_RISING_TIMING));
		battery_xlog_printk(BAT_LOG_CRTI, "[DISO] i==%d  gpio_state= %d\n", i,mt_get_gpio_in(vin_sel_gpio_number));
