...
const UINT32 BACKUP_LVDS_REG_OFFSETS[] =
{
    LVDS_TX_REG_OFFSET(LVDS_FMTCTRL),
    LVDS_TX_REG_OFFSET(LVDS_DATA_SRC),
    LVDS_TX_REG_OFFSET(LVDS_CTRL),
    
    LVDS_TX_REG_OFFSET(LVDS_R_SEL),
    LVDS_TX_REG_OFFSET(LVDS_G_SEL),
    LVDS_TX_REG_OFFSET(LVDS_B_SEL),
    
    LVDS_TX_REG_OFFSET(LVDS_OUT_CTRL),
    LVDS_TX_REG_OFFSET(LVDS_CH_SWAP),
    LVDS_TX_REG_OFFSET(LVDS_CLK_CTRL),
};

static void _BackupLVDSRegisters(void)
{


    LVDS_TX_REGS *reg = &lvdsregBackup;
    UINT32 i;
    for (i = 0; i < ARY_SIZE(BACKUP_LVDS_REG_OFFSETS); ++ i)
    {
        OUTREG32(REG_ADDR(reg, BACKUP_LVDS_REG_OFFSETS[i]),
                 AS_UINT32(REG_ADDR(LVDS_TX_REG, BACKUP_LVDS_REG_OFFSETS[i])));
    }
}

static void _RestoreLVDSRegisters(void)
{
    LVDS_TX_REGS *reg = &lvdsregBackup;
    UINT32 i;
    for (i = 0; i < ARY_SIZE(BACKUP_LVDS_REG_OFFSETS); ++ i)
    {
        OUTREG32(REG_ADDR(LVDS_TX_REG, BACKUP_LVDS_REG_OFFSETS[i]),
                 AS_UINT32(REG_ADDR(reg, BACKUP_LVDS_REG_OFFSETS[i])));
    }
}

static void _ResetBackupedLVDSRegisterValues(void)
{
    LVDS_TX_REGS *regs = &lvdsregBackup;
    memset((void*)regs, 0, sizeof(LVDS_TX_REGS));
...
if((lcm_params->dpi.lvds_tx_en == 0)  && (lcm_params->type == LCM_TYPE_DPI))
    ...
    OUTREGBIT(MIPITX_DSI_PLL_PWR_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_PWR,DA_DSI0_MPPLL_SDM_PWR_ON,1);
    OUTREGBIT(MIPITX_DSI_PLL_PWR_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_PWR,DA_DSI0_MPPLL_SDM_ISO_EN,1);
	mdelay(10);

    OUTREGBIT(MIPITX_DSI_PLL_PWR_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_PWR,DA_DSI0_MPPLL_SDM_ISO_EN,0);
	...
	if(0!=data_Rate){//if lcm_params->dpi.PLL_CLOCK=0, use other method
		if(data_Rate > 1250){
			printk("[dpi_drv.c error]Data Rate exceed limitation\n");
			ASSERT(0);
		}
			...
			printk("[dpi_drv.c Error]: dataRate is too low,%d!!!\n", __LINE__);
			ASSERT(0);
		}
		//PLL txdiv config
		switch(txdiv)
		{
			case 1:
				...
				break;
			case 2:
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,1);
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,0);
				break;
			case 4:
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,2);
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,0);
				break;
			case 8:
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,2);
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,1);
				break;
			case 16:
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,2);
                OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,2);
				break;
			default:
				break;
		}
		...
		*/
        ...
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_H,(pcw & 0x7F));
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_16_23,((256*(data_Rate*txdiv%13)/13) & 0xFF));
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_8_15,((256*(256*(data_Rate*txdiv%13)%13)/13) & 0xFF));
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_0_7,((256*(256*(256*(data_Rate*txdiv%13)%13)%13)/13) & 0xFF));

		//SSC config
//		pmod = ROUND(1000*26MHz/fmod/2);fmod default is 30Khz, and this value not be changed
//		pmod = 433.33;
        if(1 != lcm_params->dpi.ssc_disable){
            OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_PH_INIT,1);
            OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_PRD,0x1B1);//PRD=ROUND(pmod) = 433;
		if(0 != lcm_params->dpi.ssc_range){
			delta1 = lcm_params->dpi.ssc_range;
		}
		ASSERT(delta1<=8);
		pdelta1 = (delta1*data_Rate*txdiv*262144+281664)/563329;
            OUTREGBIT(MIPITX_DSI_PLL_CON3_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON3,RG_DSI0_MPPLL_SDM_SSC_DELTA,pdelta1);
            OUTREGBIT(MIPITX_DSI_PLL_CON3_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON3,RG_DSI0_MPPLL_SDM_SSC_DELTA1,pdelta1);
            //OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_FRA_EN,1);
            printk("[dpi_drv.c] PLL config:data_rate=%d,txdiv=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
			data_Rate,txdiv,INREG32(&DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2),delta1,pdelta1);
	}
    }
	else{
	
        OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,lcm_params->dpi.mipi_pll_clk_div1);
        OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,lcm_params->dpi.mipi_pll_clk_div2);

        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_H,((lcm_params->dpi.mipi_pll_clk_fbk_div)<< 2));
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_16_23,0);
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_8_15,0);
        OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_0_7,0);

        //OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_FRA_EN,0);
	}
        OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_FRA_EN,1);
...
*/
    OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_PLL_EN,1);
	mdelay(1);
    if((0 != data_Rate) && (1 != lcm_params->dpi.ssc_disable))
        OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_EN,1);
	else
        OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_EN,0);

    // default POSDIV by 4
    OUTREGBIT(MIPITX_DSI_PLL_TOP_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_TOP,RG_MPPLL_PRESERVE_L,3);
    OUTREGBIT(MIPITX_DSI_PLL_TOP_REG,DSI_PHY_REG_DPI->MIPITX_DSI_PLL_TOP,RG_MPPLL_PRESERVE_H,1);

    OUTREGBIT(MIPITX_DSI_TOP_CON_REG,DSI_PHY_REG_DPI->MIPITX_DSI_TOP_CON,RG_DSI_PAD_TIE_LOW_EN, 0);
}
...
LVDS_REG_FMTCNTL lvds_fmtctl = LVDS_TX_REG->LVDS_FMTCTRL;
	...
	if(lcm_params->dpi.lvds_tx_en == 1)
	{
	  ...
	  udelay(5);
	  OUTREG32(&LVDS_ANA_REG->LVDSTX_ANA_CTL2, 0x00c10fb7);  //1111~0000 hightest~lowest swing
	  udelay(5);
