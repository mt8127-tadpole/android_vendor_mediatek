...
#define MT8127_VCODEC_DEV_MAJOR_NUMBER 160   //189
//#define VDEC_USE_L2C

static dev_t vcodec_devno = MKDEV(MT8127_VCODEC_DEV_MAJOR_NUMBER,0);
...
static int MT8127Driver_Open_Count;         //mutex : DriverOpenCountLock
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        case VCODEC_SET_THREAD_ID:
            MFV_LOGE("[MT8127] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
            
            MFV_LOGE("[MT8127] VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_ALLOC_NON_CACHE_BUFFER:
            MFV_LOGE("[MT8127][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] + tid = %d\n", current->pid);

            MFV_LOGE("[MT8127][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_FREE_NON_CACHE_BUFFER:
            MFV_LOGE("[MT8127][M4U]! VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] + tid = %d\n", current->pid);            

            MFV_LOGE("[MT8127][M4U]! VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_INC_DEC_EMI_USER:
            MFV_LOGD("[MT8127] VCODEC_INC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_DEC_EMI_USER:
            MFV_LOGD("[MT8127] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_ENC_EMI_USER:
            MFV_LOGD("[MT8127] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_ENC_EMI_USER:
            MFV_LOGD("[MT8127] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
                ...
                rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_DEC)
            {
                while (bLockedHW == VAL_FALSE)
                {
                    ...
                    if (grVcodecDecHWLock.pvHandle == 0) // No one holds dec hw lock now
                    {
						...
						#ifdef MTK_SEC_VIDEO_PATH_SUPPORT // Jiaqiang Zhao moved to TEE
                     ...
                     rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
            {
                ...
                while (bLockedHW == VAL_FALSE)
                {
                    ...
                    if (rHWLock.u4TimeoutMs == 0)
                    {
                        if (grVcodecEncHWLock.pvHandle != 0)
                        {
                            MFV_LOGE("[MT8127 VENC]LOCKHW Early break for JPEG VEN\n");
                            break;
                        }
                    }
                    ...
                    if (EncHWLockEvent.u4TimeoutMs == 1) {
                        MFV_LOGE("[MT8127 VENC]LOCKHW[NOT ERROR][VCODEC_LOCKHW] First Use Enc HW %d!!\n", rHWLock.eDriverType);
                        FirstUseEncHW = 1;
                    }
                    else {
                        FirstUseEncHW = 0;
                    ...
                    if (grVcodecEncHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned int)rHWLock.pvHandle)) {
                        MFV_LOGE("[MT8127 VENC]LOCKHW[WARNING] one encoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%x, CurrentTID = %d, type:%d\n",
                    ...
                    if (VAL_RESULT_INVALID_ISR == eValRet) {
                        MFV_LOGE("[MT8127 VENC]LOCKHW[ERROR][VCODEC_LOCKHW] EncHWLockEvent TimeOut, CurrentTID = %d\n", current->pid);
                        if (FirstUseEncHW != 1) {
                            mutex_lock(&VencHWLock);
                            if (grVcodecEncHWLock.pvHandle == 0) {
                                MFV_LOGE("[MT8127 VENC]LOCKHW[WARNING] maybe mediaserver restart before, please check!!\n");
                            }
                            else {                                
                                MFV_LOGE("[MT8127 VENC]LOCKHW[WARNING] someone use HW, and check timeout value!! %d\n", u4VencLockTimeOutCount);
                                ++u4VencLockTimeOutCount;
                                if (u4VencLockTimeOutCount > 30)
                                {
                                    MFV_LOGE("[MT8127 VENC]LOCKHW VCODEC_LOCKHW - ID %d  fail, someone locked HW time out more than 30 times %x, %x, %x, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned int)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                                    mutex_unlock(&VencHWLock);
                                    return -EFAULT;
                                }
                                ...
                                if (rHWLock.u4TimeoutMs == 0)
                                {
                                    MFV_LOGE("[MT8127 VENC]LOCKHW VCODEC_LOCKHW - ID %d  fail, someone locked HW already %x, %x, %x, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned int)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                                    mutex_unlock(&VencHWLock);
                                    return -EFAULT;
                                }                    
                            }
                    ...
                    if (grVcodecEncHWLock.pvHandle == 0)   //No process use HW, so current process can use HW
                    {
                            ...
                            rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
                        {
                            ...
                            MFV_LOGD("[MT8127 VENC]LOCKHW[VCODEC_LOCKHW] No process use HW, so current process can use HW, handle = 0x%x\n", grVcodecEncHWLock.pvHandle);
                            grVcodecEncHWLock.eDriverType = rHWLock.eDriverType;
                            eVideoGetTimeOfDay(&grVcodecEncHWLock.rLockedTime, sizeof(VAL_TIME_T));
                    
                            MFV_LOGD("[MT8127 VENC]LOCKHWNo process use HW, so current process can use HW\n");
                            MFV_LOGD("[MT8127 VENC]LOCKHWLockInstance = 0x%x CurrentTID = %d, rLockedTime(s, us) = %d, %d\n",
                            ...
                            if (rHWLock.bSecureInst == VAL_FALSE) {
                                ...
                                {
                                    MFV_LOGE("[MT8127 VENC]LOCKHW[MFV_DEBUG][ERROR] error to request enc irq\n");
                                }
                                else
                                {
                                    MFV_LOGD("[MT8127 VENC]LOCKHW[MFV_DEBUG] success to request enc irq\n");
                                }
                                //enable_irq(MT_VDEC_IRQ_ID);
                            }
                        #else
                    ...
                    {
                        MFV_LOGE("[MT8127 VENC]LOCKHWLock ok grVcodecEncHWLock.pvHandle = 0x%x, va:%x, type:%d\n", grVcodecEncHWLock.pvHandle, (unsigned int)rHWLock.pvHandle, rHWLock.eDriverType);
                    }
                    mutex_unlock(&VencHWLock);
                }
                ...
                {
                    MFV_LOGE("[MT8127 VENC]LOCKHWVCODEC_LOCKHW - ID %d  fail, someone locked HW already , %x, %x, %x, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned int)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                    return -EFAULT;                
                }
        ...
        case VCODEC_INC_PWR_USER:
            MFV_LOGD("[MT8127] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_DEC_PWR_USER:
            MFV_LOGD("[MT8127] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
            if (ret) {
        ...
        case VCODEC_WAITISR:
            ...
            MFV_LOGD("[MT8127] VCODEC_WAITISR + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        {
            MFV_LOGE("[MT8127] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        { 
            MFV_LOGE("[MT8127] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        case VCODEC_GET_CPU_LOADING_INFO:
        {
            ...
            //MFV_LOGE("[MT8127] VCODEC_GET_CPU_LOADING_INFO num_possible_cpus(%d)\n", num_possible_cpus());
        ...
        {
            MFV_LOGD("[MT8127] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
        ...
        {
            MFV_LOGD("[MT8127] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);
        ...
        case VCODEC_SET_CPU_OPP_LIMIT:
            MFV_LOGE("[MT8127] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
...
static int vcodec_open(struct inode *inode, struct file *file)
{
    ...
    MFV_LOGE("vcodec_open pid = %d, MT8127Driver_Open_Count %d\n", current->pid, MT8127Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGE("vcodec_flush pid = %d, MT8127Driver_Open_Count %d\n", current->pid, MT8127Driver_Open_Count);

    MT8127Driver_Open_Count--;

    if (MT8127Driver_Open_Count == 0) {
...
MODULE_DESCRIPTION("MT8127 Vcodec Driver");
MODULE_LICENSE("GPL");
