...
#if 1 //#ifndef CONFIG_MT8127_EVB_BOARD
...
static void __clear_emi_mpu_vio(void)
{
    ...
    {
        printk("Error: create emi_session error %d \n", ret);
        return;
    } 
    ...
    if (ret != TZ_RESULT_SUCCESS) 
    {
        printk("[EMI] Error: close emi_session error %d \n", ret);
        return;
    }   
#else
    ...
    /* clear Device APC */
    mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_D0_VIO_STA_3)) | ABORT_EMI , DEVAPC0_D0_VIO_STA_3);
    do{
      reg_value = readl(IOMEM(DEVAPC0_D0_VIO_STA_3)) & ABORT_EMI;
      counter++;
      if(counter >= TIMEOUT)
      {
        counter = 0;
        printk(KERN_CRIT "Fail to clear DEVAPC0_D0_VIO_STA_3(EMI) violation\n");
        break;
      }
    }while (reg_value != 0);
    ...
    do{
      reg_value = readl(IOMEM(DEVAPC0_D0_VIO_STA_0)) & ABORT_EMI_BUS_INTERFACE;
      counter++;
      if(counter >= TIMEOUT)
      {
        counter = 0;
        printk(KERN_CRIT "Fail to clear DEVAPC0_D0_VIO_STA_0(EMI) violation\n");
        break;
      }
    }while (reg_value != 0);    
#endif    
...
static irqreturn_t mpu_violation_irq(int irq, void *dev_id)
{
    ...
    ret = KREE_TeeServiceCall(emi_session, TZCMD_EMI_REG, TZ_ParamTypes4(TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT), param);
    if (ret != TZ_RESULT_SUCCESS)
    {
        printk("%s error: %s\n", __FUNCTION__, TZ_GetErrorString(ret));
    }    

    mpu_reg[DAPCST0] = (uint32_t)(param[0].value.a);
    mpu_reg[DAPCST3] = (uint32_t)(param[0].value.b);
...
#else
    mpu_reg[DAPCST0] = readl(IOMEM(DEVAPC0_D0_VIO_STA_0));
    mpu_reg[DAPCST3] = readl(IOMEM(DEVAPC0_D0_VIO_STA_3));
    ...
    if ((mpu_reg[DAPCST0] & ABORT_EMI_BUS_INTERFACE) == 0 
        && (mpu_reg[DAPCST3] & ABORT_EMI) == 0) {
    ...
    __clear_emi_mpu_vio(); // move to TEE already
    #endif
...
#if 1 //#if !defined(USER_BUILD_KERNEL) || !defined(CONFIG_MT8127_EVB_BOARD)
...
static int __init emi_mpu_mod_init(void)
{
    ...
    mt65xx_reg_sync_writel(ABORT_EMI_BUS_INTERFACE, DEVAPC0_D0_VIO_STA_0);
    mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_D0_VIO_MASK_0)) & (0xFFFFFFFF ^ (1<<23)), DEVAPC0_D0_VIO_MASK_0);
    mt65xx_reg_sync_writel(ABORT_EMI, DEVAPC0_D0_VIO_STA_3);
    mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_D0_VIO_MASK_3)) & (0xFFFFFFFF ^ (1<<13)), DEVAPC0_D0_VIO_MASK_3);
    #endif
