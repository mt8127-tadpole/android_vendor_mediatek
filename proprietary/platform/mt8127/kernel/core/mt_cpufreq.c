...
* CPU stress test
***************************************************/
//#define CPUSTRESS_TURBO
//#define CPUSTRESS_VPROC_1_15V
//#define CPUSTRESS_VPROC_1_25V_GPU_500MHZ_1BUCK

/**************************************************
* If MT6333/MT6397 supported, VPROC could support lower than 1.15V
...
***************************************************/
#if 0 //#ifdef MTK_SDIOAUTOK_SUPPORT  //Check MT8127 timing result
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[], unsigned int array_size)
{
    ...
    if((g_cpufreq_get_ptp_level >= 0) && (g_cpufreq_get_ptp_level <= 4))
    {
        ...
        mt_cpufreq_reg_write(pmic_volt[2], PMIC_WRAP_DVFS_WDATA_array[7]);
        #endif
    }
    else if((g_cpufreq_get_ptp_level >= 5) && (g_cpufreq_get_ptp_level <= 6))  
    {
        ...
        #ifndef MT_DVFS_LOW_VOLTAGE_SUPPORT
        mt_cpufreq_reg_write(pmic_volt[1], PMIC_WRAP_DVFS_WDATA_array[7]);
        #endif    
    }
    else
    {
    ...
    if((g_cpufreq_get_ptp_level >= 0) && (g_cpufreq_get_ptp_level <= 4))
    {
        ...
        #ifndef MT_DVFS_LOW_VOLTAGE_SUPPORT
        mt_cpufreq_pmic_volt[7] = pmic_volt[2];
        #endif
    }
    else if((g_cpufreq_get_ptp_level >= 5) && (g_cpufreq_get_ptp_level <= 6))  
    {
        ...
        #ifndef MT_DVFS_LOW_VOLTAGE_SUPPORT
        mt_cpufreq_pmic_volt[7] = pmic_volt[1];
        #endif 
    }
    else
    {
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
    ...
    /* Assume mt8127_freqs_e1 voltage will be put in order, and freq will be put from high to low.*/
...
void mt_cpufreq_volt_set_by_sdio(unsigned int volt)
{
	...
	{
        #if MTK_PMIC_MT6397
		pmic_config_interface(0x226,reg,0xFFFF,0); 
		pmic_config_interface(0x228,reg,0xFFFF,0);
        #else
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    *************************************************/
    ...
    if(g_cpufreq_get_ptp_level == 0)
#ifdef CPUSTRESS_VPROC_1_25V_GPU_500MHZ_1BUCK
        next.cpufreq_khz = mt8127_freqs_e1_gpu[idx].cpufreq_khz;
#else
        next.cpufreq_khz = mt8127_freqs_e1[idx].cpufreq_khz;
#endif
...
#ifdef CPUSTRESS_VPROC_1_25V_GPU_500MHZ_1BUCK
        next.cpufreq_khz = mt8127_freqs_e1_3_gpu[idx].cpufreq_khz;
#else
    ...
    *************************************************/
    #ifndef MT_DVFS_LOW_VOLTAGE_SUPPORT
    if ( (num_possible_cpus() == 4) && (num_online_cpus() < num_possible_cpus()) && (freqs.new > DVFS_F2))
    {
        if (subsys_is_on(SYS_ISP) || subsys_is_on(SYS_VDE)) //(isp_vdec_on_off() == true)
        {
...
#if 0
    ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1));
#else
    if(g_cpufreq_get_ptp_level == 0)
#ifdef CPUSTRESS_VPROC_1_25V_GPU_500MHZ_1BUCK
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_gpu));
#else
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1));
#endif
    else if(g_cpufreq_get_ptp_level == 1)
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_1));
    else if(g_cpufreq_get_ptp_level == 2)
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_2));
    else if(g_cpufreq_get_ptp_level == 3)
#ifdef CPUSTRESS_VPROC_1_25V_GPU_500MHZ_1BUCK
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_3_gpu));
#else
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_3));
#endif
    else if(g_cpufreq_get_ptp_level == 4)
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_4));
    else if(g_cpufreq_get_ptp_level == 5)
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_5));
    else if(g_cpufreq_get_ptp_level == 6)
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1_6));
    else
        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt8127_freqs_e1));
#endif
...
void mt_cpufreq_return_default_DVS_by_ptpod(void)
{
    ...
    if(g_cpufreq_get_vcore_corner == false)
    {
        mt_cpufreq_reg_write(0x18, PMIC_WRAP_DVFS_WDATA4); // 0.85V VPROC, for spm control in deep idle
        mt_cpufreq_reg_write(0x48, PMIC_WRAP_DVFS_WDATA5); // 1.15V VPROC, for spm control in deep idle
        mt_cpufreq_reg_write(0x38, PMIC_WRAP_DVFS_WDATA6); // 1.05V VCORE, for spm control in deep idle
        mt_cpufreq_reg_write(0x48, PMIC_WRAP_DVFS_WDATA7); // 1.15V VCORE, for spm control in deep idle
        
        /* For PTP-OD */
        mt_cpufreq_pmic_volt[4] = 0x18; // 0.85V VPROC, for spm control in deep idle
        mt_cpufreq_pmic_volt[5] = 0x48; // 1.15V VPROC, for spm control in deep idle
        mt_cpufreq_pmic_volt[6] = 0x38; // 1.05V VCORE, for spm control in deep idle
        mt_cpufreq_pmic_volt[7] = 0x48; // 1.15V VCORE, for spm control in deep idle
    }
    else
    {
        mt_cpufreq_reg_write(0x18, PMIC_WRAP_DVFS_WDATA4); // 0.85V VPROC, for spm control in deep idle
        ...
        mt_cpufreq_reg_write(0x38, PMIC_WRAP_DVFS_WDATA6); // 1.05V VCORE, for spm control in deep idle
        mt_cpufreq_reg_write(0x4E, PMIC_WRAP_DVFS_WDATA7); // 1.185V VCORE, for spm control in deep idle

        /* For PTP-OD */
        mt_cpufreq_pmic_volt[4] = 0x18; // 0.85V VPROC, for spm control in deep idle
        mt_cpufreq_pmic_volt[5] = 0x4E; // 1.185V VPROC, for spm control in deep idle
        mt_cpufreq_pmic_volt[6] = 0x38; // 1.05V VCORE, for spm control in deep idle
        mt_cpufreq_pmic_volt[7] = 0x4E; // 1.185V VCORE, for spm control in deep idle
    }
    #else
    mt_cpufreq_reg_write(0x18, PMIC_WRAP_DVFS_WDATA4); // 0.85V VPROC, for spm control in deep idle
    mt_cpufreq_reg_write(0x48, PMIC_WRAP_DVFS_WDATA5); // 1.15V VPROC, for spm control in deep idle
    ...
    mt_cpufreq_pmic_volt[6] = 0x38; // 1.05V VCORE, for spm control in deep idle
    mt_cpufreq_pmic_volt[7] = 0x48; // 1.15V VCORE, for spm control in deep idle
    #endif //#ifdef CPUFREQ_SDIO_TRANSFER
...
static ssize_t mt_cpufreq_ptpod_test_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if (sscanf(buffer, "%d", &enable) == 1)
    {
        if (enable == 0) 
        {
            ...
            mt_cpufreq_ptpod_test[4] = 0x18; // 0.85V VPROC, for spm control in deep idle
            ...
            mt_cpufreq_ptpod_test[4] = 0x38; // 1.05V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[5] = 0x48; // 1.15V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[6] = 0x38; // 1.05V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[7] = 0x48; // 1.15V VPROC, for spm control in deep idle
            #endif
        ...
        else if (enable == 1)
        {
            ...
            mt_cpufreq_ptpod_test[4] = 0x17; // 0.85V VPROC, for spm control in deep idle
            ...
            mt_cpufreq_ptpod_test[4] = 0x37; // 1.05V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[5] = 0x47; // 1.15V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[6] = 0x37; // 1.05V VPROC, for spm control in deep idle
            mt_cpufreq_ptpod_test[7] = 0x47; // 1.15V VPROC, for spm control in deep idle
            #endif
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
    ...
    #ifdef CPUSTRESS_TURBO
    g_cpufreq_get_ptp_level = 3;
    #endif
    ...
    *************************************************/
    ...
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR0);
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR1);
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR2);
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR3);
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR4); // VPROC
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR5); // VPROC
    #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
    mt_cpufreq_reg_write(0x027A, PMIC_WRAP_DVFS_ADR6); // VCORE
    mt_cpufreq_reg_write(0x027A, PMIC_WRAP_DVFS_ADR7); // VCORE
    #else
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR6); // VPROC
    mt_cpufreq_reg_write(0x0228, PMIC_WRAP_DVFS_ADR7); // VPROC
    #endif
#else
    ...
    mt_cpufreq_reg_write(0x58, PMIC_WRAP_DVFS_WDATA1); // 1.25V VPROC
    mt_cpufreq_reg_write(0x58, PMIC_WRAP_DVFS_WDATA2); // 1.25V VPROC
    #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
    mt_cpufreq_reg_write(0x38, PMIC_WRAP_DVFS_WDATA3); // 1.05V VPROC
    #else
    mt_cpufreq_reg_write(0x58, PMIC_WRAP_DVFS_WDATA3); // 1.25V VPROC		 
    #endif
			
    mt_cpufreq_pmic_volt[0] = 0x58; // 1.25V VPROC
    mt_cpufreq_pmic_volt[1] = 0x58; // 1.25V VPROC
    mt_cpufreq_pmic_volt[2] = 0x58; // 1.25V VPROC
    #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
    mt_cpufreq_pmic_volt[3] = 0x38; // 1.05V VPROC
    #else
    mt_cpufreq_pmic_volt[3] = 0x58; // 1.25V VPROC
    #endif
#endif
