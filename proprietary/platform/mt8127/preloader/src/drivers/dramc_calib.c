...
#if COMBO_PCDDR3 > 0
//PCDDR3X16
unsigned char Bit_DQO_Mapping_ddr3_x16[32] = 
{
    8,  9,  10, 11, 4,  5,  6,  7,   //Bit0~7
    0,  1,  2,  3,  12, 13, 14, 15,  //Bit8~15
    24, 25, 26, 27, 20, 21, 22, 23,  //Bit16~23
    16, 17, 18, 19, 28, 29, 30, 31   //Bit24~31
};
unsigned char DQSO_Mapping_ddr3_x16[4] = {0, 1, 2, 3}; 	// This could be by DQSx for CPU/CMP_ERR bit 0~7, bit 8~15, bit 16~23, bit 24~31
unsigned char DQM_Mapping_ddr3_x16[4] = {0, 1, 2, 3};

//PCDDR3X8
unsigned char Bit_DQO_Mapping_ddr3_x8[32] = 
{
    0,  1,  2,  3,  4,  5,  6,  7, //Bit0~7
    8,  9,  10, 11, 12, 13, 14, 15, //Bit8~15
    16, 17, 18, 19, 20, 21, 22, 23, //Bit16~23
...
unsigned char DQSO_Mapping_ddr3_x8 = {0, 1, 2, 3}; 	// This could be by DQSx for CPU/CMP_ERR bit 0~7, bit 8~15, bit 16~23, bit 24~31
unsigned char DQM_Mapping_ddr3_x8 = {0, 1, 2, 3};
#endif
//--------------------------------------------------------------------------------------------
...
#ifndef BYPASS_DDR3_AUTO_DETECT
...
#define ABORT_EMI               0x00002000 //DEVAPC0_D0_VIO_STA_3, idx:13
static void clear_emi_mpu_vio(void)
{
    ...
    reg_write32(DEVAPC0_D0_VIO_STA_3, ABORT_EMI);
#if 0
    print("[EMI]EMI_MPUS = 0x%x\n", DRV_Reg32(EMI_MPUS));
    print("[EMI]EMI_MPUT = 0x%x\n", DRV_Reg32(EMI_MPUT));
    
    print("[EMI]EMI_MPUP = 0x%x\n", DRV_Reg32(EMI_MPUP));
    print("[EMI]EMI_MPUQ = 0x%x\n", DRV_Reg32(EMI_MPUQ));
    print("[EMI]EMI_MPUR = 0x%x\n", DRV_Reg32(EMI_MPUR));
    print("[EMI]EMI_MPUY = 0x%x\n", DRV_Reg32(EMI_MPUY));
    print("[EMI]DEVAPC0_D0_VIO_STA_3 = 0x%x\n", reg_read32(DEVAPC0_D0_VIO_STA_3));
#endif
}
...
void ddr3_rank_size_detect(void)
{
	...
	*(volatile unsigned *)(EMI_CONA) = emi_cona & ~0x20000;

#if 0
	if(RANK_CURR == 1)
	{
		emi_set->DRAM_RANK_SIZE[1] = emi_set->DRAM_RANK_SIZE[0];
		print("[EMI]rank%d size: 0x%x\n", RANK_CURR, emi_set->DRAM_RANK_SIZE[1]);
		return;
	}
#endif
	...
	*((volatile unsigned int *)start_addr) = u4pattern;
	...
	emi_set->DRAM_RANK_SIZE[RANK_CURR] = real_rank_size;
	print("[EMI]rank%d size: 0x%x\n", RANK_CURR, real_rank_size);

	/*restore rank mode*/
	*(volatile unsigned *)(EMI_CONA) = emi_cona;
...
int do_sw_rx_dq_dqs_calib(void)
{
    ...
    for(dq_tap = 0 ; dq_tap < MAX_RX_DQDLY_TAPS; dq_tap++ ){
        ...
        if(dq_tap==0)
        {
            dbg_print("do_sw_rx_dq_dqs_calib Test Agent 2 enable\n");
        }
        ...
        if(dq_tap==0)
        {
            dbg_print("do_sw_rx_dq_dqs_calib Test Agent 2 done\n");
        }
...
#if (defined(DQ_DQS_DQM_REMAPPING) && (COMBO_PCDDR3>0))
    unsigned char *Bit_DQO_Mapping;
    unsigned char *DQSO_Mapping;
    unsigned char *DQM_Mapping;
    if((DRAMC_READ_REG(0x00d8)>>30)==0x01)//ddr3 8bit pinmux
    {
        Bit_DQO_Mapping = (unsigned char *)Bit_DQO_Mapping_ddr3_x8;
        DQSO_Mapping = (unsigned char *)DQSO_Mapping_ddr3_x8;
        DQM_Mapping = (unsigned char *)DQM_Mapping_ddr3_x8;
    }
    else
    {
        Bit_DQO_Mapping = (unsigned char *)Bit_DQO_Mapping_ddr3_x16;
        DQSO_Mapping = (unsigned char *)DQSO_Mapping_ddr3_x16;
        DQM_Mapping = (unsigned char *)DQM_Mapping_ddr3_x16;
    }
#endif
...
//#define DEBUG_DRAMC_CALIB
    ...
    *((volatile unsigned *)(DRAMC0_BASE + 0x04)) = (*((volatile unsigned *)(DRAMC0_BASE + 0x04)) & (~(0x3 << 8)))| (rank0_col << 8);
    ...
    {
        /*cona & conf1 maybe change because bitwidth detect*/
        bak_cona = *(volatile unsigned *)(EMI_CONA);
        bak_conf1 = *((volatile unsigned *)(DRAMC0_BASE + 0x04));
    }
#endif
    ...
    if (ddr3_autodetect_is_enable())
    {
        ...
        dqsi_gw_dly_coarse_factor_handler_rank1(opt_gw_coarse_value0);
        dqsi_gw_dly_fine_factor_handler_rank1(opt_gw_fine_value0);
        DRAMC_WRITE_REG(opt_rx_dqs0,DRAMC_R1DELDLY);

        *(volatile unsigned int *)(0x80000000) = 0x12345678;
        *(volatile unsigned int *)(0x80000004) = ~0x12345678;
        if((*(volatile unsigned int *)(0x80000000) == 0x12345678) &&
           (*(volatile unsigned int *)(0x80000004) == ~0x12345678))
        {
            print("[EMI]rank auto detect:==dual rank==\n");
        }
        else
        {
            print("[EMI]warning:rank auto detect:==single rank==\n");
...
dramc_calib_back_to_rank0:
    ...
    *((volatile unsigned *)(DRAMC0_BASE + 0x04))= bak_conf1;
    ...
    if(ddr3_autodetect_is_enable())
    {
        //QW: single rank && 16bit x1pcs => treat rank bit as row bit 15
        if (( *(volatile unsigned *)(EMI_CONA) & 0x20002) == 0)
            *(volatile unsigned *)(EMI_TESTB) |= 0x80; 
    }
#endif
