...
/* INTERNAL FUNCTIONS                                                         */
/*============================================================================*/
/* for loading mtee image case, the dram size for boot args needs to be adjusted */
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT
/* adjust dram size */
u32 g_adjust_dram_sz_for_boot_args = 0;

/* adjust dram size for boot args */
platform_adjust_dram_sz_in_boot_args
(
unsigned int sz
)
{
#if CFG_BOOT_ARGUMENT
    int i=0;
    if( 0 == bootarg.dram_rank_num )
    {
        print("%s Error: dram rank number is zero in boot args\n", MOD);
        return;
    }

    for(i = bootarg.dram_rank_num-1; i >= 0; i-- )
    {
        if( bootarg.dram_rank_size[i] >= sz )
        {
             bootarg.dram_rank_size[i] -= sz;
             print("%s Succedd to adjust dram size in boot args\n", MOD);
             return;
        }
        else
        {
            sz -= bootarg.dram_rank_size[i];
            bootarg.dram_rank_size[i] = 0;
        }
    }

    print("%s Error: external memory size is smaller than the corrected size\n", MOD);
    return;
#else
    return;
#endif
}
#endif /* CFG_MTK_IN_HOUSE_TEE_SUPPORT */
...
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT
    platform_adjust_dram_sz_in_boot_args(g_adjust_dram_sz_for_boot_args);
#endif
...
void platform_pre_init(void)
{
    ...
    /* EFUSE SW workaround */
    while( (DRV_Reg32(EFUSEC_CON) & (0x1 << 1)) != 0 ) {
        platform_cnt++;
        if(platform_cnt % 100 == 99) {
            print("Platform is still busy, already poll %d times...\n", platform_cnt + 1);
        }
    }
    platform_cnt = 0;
    DRV_SetReg32(EFUSEC_CON, (0x1 << 2));
    print("Platform initialization is ok\n");
...
//enable long press reboot function***************
    ...
    #ifdef MTK_PMIC_MT6397
	{
		...
		if ((reg_val & 0x02) == 0x02)
            ...
            #ifdef MTK_PMIC_MT6397
			pmic_config_interface(GPIO_SMT_CON3, 0x01, PMIC_RG_HOMEKEY_PUEN_MASK, PMIC_RG_HOMEKEY_PUEN_SHIFT);//pull up homekey
            #else
...
//#if 1 /* FIXME */
#if !CFG_FPGA_PLATFORM && !defined MTK_ALPS_BOX_SUPPORT  /* FPGA workaround */
