...
u64 part_get_startaddress(u64 byte_address)
{
    ...
    while (index < PART_MAX_COUNT) {
		if(lastest_part[index].offset > byte_address)
...
static int load_pt_from_fixed_addr(u8 *buf)
{
          ...
          //if(false == bSecRead)
          {
              printf("%s scan pmt from %x\n", __func__,pt_start);
          }
          /* try to find the pmt at fixed address, signature:0x50547631 */

              dev->bread(dev,pt_start/PAGE_SIZE,buffer_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
             	...
             	if(is_valid_pt(page_buf)&&is_valid_pt(&page_buf[pt_size-4])){                
                  ...
                  memcpy(page_readbuf,&page_buf[PT_SIG_SIZE],pt_size-PT_SIG_SIZE);
              	 ...
              	 dev->bread(dev,mpt_start/PAGE_SIZE,buffer_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
 ...
 find:
	...
	if(pt_next == 0x1)
	{
		 dev->bread(dev,(pt_addr+pt_size)/PAGE_SIZE,pt_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
...
int load_exist_part_tab(u8 *buf)
{
            ...
            {
                buf_p = page_buf;
                dev->bread(dev,(pt_start + i*buffer_size)/PAGE_SIZE,buffer_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
            ...
            //  printf("search %x %x\n",buf_p,pt_start + i*4096+j*PAGE_SIZE);
                    if(is_valid_pt(buf_p)){
                        ...
                        if((buffer_size-j*PAGE_SIZE) < pt_size){
                            ...
                            dev->bread(dev,(pt_start + (i+1)*buffer_size)/PAGE_SIZE,pt_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
            ...
            if(i == CFG_EMMC_PMT_SIZE/buffer_size)
            {
                ...
                buf_p = page_buf;
                dev->bread(dev,(mpt_start + i*buffer_size)/PAGE_SIZE,buffer_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
                ...
                for(j=0;j<buffer_size/PAGE_SIZE;j++){
                    ...
                    if(is_valid_mpt(buf_p)){
                        ...
                        if((buffer_size - j*PAGE_SIZE) < pt_size){
                            ...
                            memcpy(page_readbuf,&buf_p[PT_SIG_SIZE],len-PT_SIG_SIZE);
                            dev->bread(dev,(mpt_start + (i+1)*buffer_size)/PAGE_SIZE,pt_size/PAGE_SIZE,(u8*)page_buf, EMMC_PART_USER);
