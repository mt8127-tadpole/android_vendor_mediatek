...
int g_max_gw_score = 0;
unsigned int ddr3_autodetect_is_enable(void);

void ddr3_x16_x32_auto_adjust(void)
{

	unsigned int u4reg_8c = 0;

#if (defined(CFG_CUSTOM_DRAM_SIZE_SUPPORT) && defined(DDR3_FIXED_16B))
	g_max_gw_score = 2; //fixed as 16bit
	print("[EMI]Fix g_max_gw_score = %d\n", g_max_gw_score);
#endif

	if((g_max_gw_score == 1) || (g_max_gw_score == 2))
	{
		print("[EMI]warning: PCDDR3 x16, gw_score = %d!!\n", g_max_gw_score);
		*(volatile unsigned int*)(EMI_CONA) &= ~(0x1<<1);
		*((volatile unsigned int *)(EMI_BASE + 0x00e8))&= ~(0x1<<5);

		*(volatile unsigned int *)(DRAMC0_BASE + 0x0004) &= ~(0x1<<0);
		*(volatile unsigned int *)(DDRPHY_BASE + 0x0004) &= ~(0x1<<0);

		if(g_max_gw_score == 2)
			u4reg_8c = 0x00100008;
		
		*(volatile unsigned int *)(DRAMC0_BASE + 0x008C) = u4reg_8c;
		*(volatile unsigned int *)(DDRPHY_BASE + 0x008C) = u4reg_8c;
	}
...
#ifdef DDR3_AUTO_DETECT_SUPPORT
int Do_Read_Test_DDR3_with_autodetect(unsigned int start, unsigned int len, void *ext_arg)
{//for DDR3 x16 and DDR3 x32 gating counter
   int score = -1;

   Sequence_Read(start,len,ext_arg);

	if((DRAMC_READ_REG(DRAMC_DQSGNWCNT0)== 0x02020202) && (DRAMC_READ_REG(DRAMC_DQSGNWCNT1)== 0x02020202))
	{
		score = 3;
	}
	else if(DRAMC_READ_REG(DRAMC_DQSGNWCNT0)== 0x02020202)
	{
		score = 1;
	}
	else if(DRAMC_READ_REG(DRAMC_DQSGNWCNT1)== 0x02020202)
	{
		score = 2;
	}
...
int cpu_read_test(unsigned int start, unsigned int len, void *ext_arg){
    ...
    #ifdef DDR3_AUTO_DETECT_SUPPORT
    {
        if(ddr3_autodetect_is_enable())
            err = Do_Read_Test_DDR3_with_autodetect(start, 1, ext_arg);
        else
...
static unsigned int __ett_print_dqs_gw_result(test_case *test_cases, int nr_ts)
{
	...
	if(ddr3_autodetect_is_enable())
	{
		ddr3_score =  test_cases[i].test_case(test_cases[i].start, test_cases[i].range, test_cases[i].ext_arg);
		if(ddr3_score < 0)
		{
			dqs_print("    0");
			score = 0;
			...
			score = (unsigned int)ddr3_score;
	}
	else
   	{
   		//keep orig flow for non-PCDDR3
	...
	if(ddr3_autodetect_is_enable())
	{
		if (score != 0) {
			...
			if(score > g_max_gw_score)
			{
				g_max_gw_score = score;
				dqs_gw[dqs_gw_coarse] = (1 << dqs_gw_fine); 
			} 
			else if(score == g_max_gw_score)
			{
...
/*FIX GW setting*/
#if 0//def DQS_GW_COARSE_FIXED
    opt_gw_coarse_value1 = DQS_GW_COARSE_FIXED;
    dqsi_gw_dly_coarse_factor_handler(opt_gw_coarse_value1);
    
    opt_gw_fine_value1 = DQS_GW_FINE_FIXED;
    dqsi_gw_dly_fine_factor_handler(opt_gw_fine_value1);
    print("FIX GW setting~~~\n");
#endif

 #ifdef DDR3_AUTO_DETECT_SUPPORT
    if(ddr3_autodetect_is_enable())
        ddr3_x16_x32_auto_adjust();
 #endif
