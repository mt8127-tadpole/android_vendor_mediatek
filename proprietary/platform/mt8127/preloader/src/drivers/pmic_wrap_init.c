...
/******************************************************************************
 * pmic_wrapper.c - MT8217 Linux pmic_wrapper Driver
 *
...
static S32 _pwrap_init_dio( U32 dio_en )
{
  ...
  WRAP_WR32(PMIC_WRAP_HIPRIO_ARB_EN , 0x4); // only WACS2
...
static S32 _pwrap_init_reg_clock( U32 regck_sel )
{
  ...
  if( regck_sel == 1 )
  { // 6MHz in 6323  => no support ; // 12MHz in 6397
#ifdef SLV_6397
    WRAP_WR32(PMIC_WRAP_CSHEXT_WRITE   , 0x0);  // wait data written into register => 3T_PMIC
    WRAP_WR32(PMIC_WRAP_CSHEXT_READ    , 0x4);  // for 6320, slave need enough time (4T of PMIC reg_ck) to back idle state
  ...
  { // 12MHz in 6323; 24MHz in 6397
#ifdef SLV_6397
    WRAP_WR32(PMIC_WRAP_CSHEXT_WRITE   , 0x4);  // wait data written into register => 3T_PMIC: consists of CSLEXT_END(1T) + CSHEXT(6T)
...
S32 pwrap_init ( void )
{
  ...
  //Reset SPISLV
  //###############################
#if !CFG_FPGA_PLATFORM	/* FPGA workaround */
  ...
  WRAP_WR32(PMIC_WRAP_HIPRIO_ARB_EN,4); //Only WACS2
  ...
  //###############################
#ifdef SLV_6397
  /* For low power: Only Enable WRAP_PDN */
  pwrap_read_nochk(PMIC_WRP_CKPDN, &rdata);
  sub_return= pwrap_write_nochk(PMIC_WRP_CKPDN, (rdata & 0x7F));//set dewrap clock bit
  ...
  /* Add patch here */
  /* Enable SPI bus SMT */
  pwrap_read_nochk(PMIC_GPIO_SMT_CON0, &rdata);
  sub_return= pwrap_write_nochk(PMIC_GPIO_SMT_CON0, (rdata | 0x01E0));
  pwrap_read_nochk(PMIC_GPIO_SMT_CON0, &rdata);
  PWRAPLOG("recheck PMIC_GPIO_SMT_CON0 data, data=0x%x \n", rdata);
  if( sub_return != 0)
  {
    PWRAPERR("Enable PMIC_GPIO_SMT_CON0 fail, sub_return=%x s\n", sub_return1);
    return E_PWR_INIT_ENABLE_PMIC;
  }
#endif
