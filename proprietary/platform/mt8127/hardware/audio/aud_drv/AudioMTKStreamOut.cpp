...
status_t AudioMTKStreamOut::SetPlayBackPinmux()
{
    ...
    AnalogBlockAttributeParameters mAnalogBlockAttributeParameters;
    if ((DlDevice & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (DlDevice & AUDIO_DEVICE_OUT_WIRED_HEADPHONE))
    {
        mAnalogBlockAttributeParameters.device_type = AudioAnalogType::DEVICE_OUT_HEADSETR;
        mAnalogBlockAttributeParameters.mux_type = AudioAnalogType::MUX_AUDIO;
        mAudioAnalogControl->setParameters(AudioAnalogType::SET_MUX_ATTRIBUTE, &mAnalogBlockAttributeParameters);

        mAnalogBlockAttributeParameters.device_type = AudioAnalogType::DEVICE_OUT_HEADSETL;
        mAudioAnalogControl->setParameters(AudioAnalogType::SET_MUX_ATTRIBUTE, &mAnalogBlockAttributeParameters);

        mAnalogBlockAttributeParameters.device_type = AudioAnalogType::DEVICE_OUT_SPEAKERR;
        mAudioAnalogControl->setParameters(AudioAnalogType::SET_MUX_ATTRIBUTE, &mAnalogBlockAttributeParameters);

        mAnalogBlockAttributeParameters.device_type = AudioAnalogType::DEVICE_OUT_SPEAKERL;
        mAudioAnalogControl->setParameters(AudioAnalogType::SET_MUX_ATTRIBUTE, &mAnalogBlockAttributeParameters);        
    }
    if (DlDevice & AUDIO_DEVICE_OUT_EARPIECE)
    {
        mAnalogBlockAttributeParameters.device_type = AudioAnalogType::DEVICE_OUT_EARPIECEL;
        mAnalogBlockAttributeParameters.mux_type = AudioAnalogType::MUX_VOICE;
        mAudioAnalogControl->setParameters(AudioAnalogType::SET_MUX_ATTRIBUTE, &mAnalogBlockAttributeParameters);    
    }
...
void AudioMTKStreamOut::dokeyRouting(uint32_t new_device)
{
    ...
    if (new_device & AUDIO_DEVICE_OUT_AUX_DIGITAL)
    {
        new_device &= ~AUDIO_DEVICE_OUT_AUX_DIGITAL;
        ALOGW("%s() ignore unsupported device AUX_DIGITAL", __FUNCTION__);
    }
#endif
    ...
    if (param.getInt(String8("force_standby"), value) == NO_ERROR) // only set this param when entering call related mode!!
    {
        if (value == true)
        {
            ...
            else if (AudioMATVController::GetInstance()->GetMatvEnable() == true)
            {
                ...
                AudioMATVController::GetInstance()->SetMatvEnable(false, AudioMATVController::GetInstance()->GetMatvType());
            }
            standby();
        }
        else
