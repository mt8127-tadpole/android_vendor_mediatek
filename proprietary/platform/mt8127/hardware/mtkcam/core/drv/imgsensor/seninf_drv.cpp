...
int SeninfDrvImp::init()
{
    ...
    mpCAMIODrvRegAddr = mpGpioHwRegAddr + (0xB60 / 4);//GPIO119 -> CMMCLK
...
//        *pCMMCLKReg = ( (*pCMMCLKReg)&(~0x1C0) ) | 0x40;
   ...
   //set CMMCLK mode 1 (GPIO119)
    temp = *(mpGpioHwRegAddr + (0x770/4));
    mt65xx_reg_writel(((temp&0x0FFF)|0x1000), mpGpioHwRegAddr + (0x770/4));       
...
int SeninfDrvImp::uninit()
{
    ...
    if (mUsers == 0) {
        ...
        //set CMMCLK mode 0 (GPIO119)
        *(mpGpioHwRegAddr + (0x770/4)) &= 0x0FFF;   
...
int SeninfDrvImp::initTg1CSI2(bool csi2_en)
{
   ...
   LOG_MSG("[initCSI2]:enable = %d\n", (int) csi2_en);
	...
	if(csi2_en == 0) {
        ...
        //mode 2 for parallel pin  (GPIO107-116)
        // 0x750 for 107-109
        temp = *(mpGpioHwRegAddr + (0x750/4));
        mt65xx_reg_writel(((temp&0x003F)|0x2480), mpGpioHwRegAddr + (0x750/4)); 
        // 0x760 for 110-114
        temp = *(mpGpioHwRegAddr + (0x760/4));
        mt65xx_reg_writel(((temp&0x0000)|0x2492), mpGpioHwRegAddr + (0x760/4)); 
        // 0x770 for 115-116
        temp = *(mpGpioHwRegAddr + (0x770/4));
        mt65xx_reg_writel(((temp&0xFFC0)|0x12), mpGpioHwRegAddr + (0x770/4)); 
        
        //mode 1 for parallel pin (GPIO117-120)
        // 0x770 for 117-119
        temp = *(mpGpioHwRegAddr + (0x770/4));
	#if 0	
        //mt65xx_reg_writel(((temp&0x03F)|0x1240), mpGpioHwRegAddr + (0x770/4)); // GPIO #118: CMDATA1 & GPS share the same pin, Do NOT change its default settings
        mt65xx_reg_writel(((temp&0x0E3F)|0x1040), mpGpioHwRegAddr + (0x770/4)); 
    #else //revise for BESTONE : GPIO #117 CMDATA0 & LCM share the same pin
	    mt65xx_reg_writel(((temp&0x0E3F)|0x1000), mpGpioHwRegAddr + (0x770/4));
	#endif

        // 0x780 for 120
        temp = *(mpGpioHwRegAddr + (0x780/4));
        mt65xx_reg_writel(((temp&0xFFF8)|0x1), mpGpioHwRegAddr + (0x780/4)); 
       ...
       mt65xx_reg_writel(temp&0xFFFFFFE7, mpCSI2RxAnalogRegAddr + (0x10/4));
        ...
        if(mDevice & SENSOR_DEV_MAIN) {
            ...
            //mode 1   (GPIO101-110)
            //0x740 for 101-104
            temp = *(mpGpioHwRegAddr + (0x740/4)); 
            mt65xx_reg_writel(((temp&0x0007)|0x1248), mpGpioHwRegAddr + (0x740/4));     
            //0x750 for 105-109
            temp = *(mpGpioHwRegAddr + (0x750/4)); 
            mt65xx_reg_writel(((temp&0x0000)|0x1249), mpGpioHwRegAddr + (0x750/4));
            //0x760 for 110
            temp = *(mpGpioHwRegAddr + (0x760/4)); 
            mt65xx_reg_writel(((temp&0xFFF8)|0x0001), mpGpioHwRegAddr + (0x760/4));         
           ...
           mt65xx_reg_writel(((temp&0xFFFFFF)|0xE4000000), mpCSI2RxConfigRegAddr + (0x24/4));             
            ...
            //0x760 for 111-114
            temp = *(mpGpioHwRegAddr + (0x760/4)); 
            mt65xx_reg_writel(((temp&0x0007)|0x1248), mpGpioHwRegAddr + (0x760/4));
            //0x770 for 115-116
            temp = *(mpGpioHwRegAddr + (0x770/4)); 
            mt65xx_reg_writel(((temp&0xFFC0)|0x0009), mpGpioHwRegAddr + (0x770/4));                   
    ...
    unsigned long conti_mode, unsigned long csd_num)        
{
    ...
    //set CMCSD0,1(gpio_117,118) mode 2
    //set CMPCLK(gpio_120) mode 1
    unsigned int temp = 0;
    // 0x770 for 117-118
    temp = *(mpGpioHwRegAddr + (0x770/4));
    mt65xx_reg_writel(((temp&0xF03F)|0x0480), mpGpioHwRegAddr + (0x770/4)); 
    // 0x780 for 120
    temp = *(mpGpioHwRegAddr + (0x780/4));
    mt65xx_reg_writel(((temp&0xFFF8)|0x0001), mpGpioHwRegAddr + (0x780/4));
