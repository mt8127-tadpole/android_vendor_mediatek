...
u32 mtk_nand_page_transform(u64 logical_address, u32* blk, u32* map_blk)
{
	...
	//			while(1);
			...
			page_in_block = (int)(((logical_address-start_address) /g_nand_chip.page_size)% ((1 << (g_nand_chip.phys_erase_shift-g_nand_chip.page_shift))));
...
//				page_in_block = devinfo.feature_set.PairPage[page_in_block];
		...
		else
		{
			...
			mapped_block = get_mapping_block_index(block);
			page_in_block = (int)((logical_address/g_nand_chip.page_size) % (BLOCK_SIZE >> g_nand_chip.page_shift));
		}
