    ...
    while(1)
    {
        ...
        if (!is_early_suspended){
            ...
            if(DISP_THREAD_TIMEOUT < ((unsigned int)disp_thread_cost_time/1000)){
                printk("dete_hang:%lld,%lld,%lld (%d)\n",disp_thread_start_time, sched_clock(),disp_thread_cost_time, disp_thread_timeout_cnt);
                disp_thread_timeout_cnt++;
            }
            else
                disp_thread_timeout_cnt = 0;

            printk("[_DISP_DetectAliveKThread] vsync = %d, disp_thread = %d, disp_thread_cost = %lld ns, wait_mem = %d\n",
                vsync_timeout_cnt,disp_thread_timeout_cnt,disp_thread_cost_time, disp_memout_timeout_cnt);

            ////wait Vsync to check if vsync is alive?
            vsync_wq_flag = 0;
            if (wait_event_interruptible_timeout(vsync_wq, vsync_wq_flag, HZ) == 0)
            {
                printk("[_DISP_DetectAliveKThread] Wait VSync timeout. early_suspend=%d, HZ=%d\n", is_early_suspended, HZ);
                vsync_timeout_cnt++;
            }
            else
                vsync_timeout_cnt = 0;
        ...
        else
        {
            ...
            vsync_timeout_cnt = 0;
            disp_thread_timeout_cnt = 0;
            printk("[_DISP_DetectAliveKThread] reset count when early suspend\n");
        }
