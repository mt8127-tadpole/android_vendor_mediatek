...
#define MTK_AVI_ADD_CODEC_SUPPORT   1
#endif
...
#if MTK_AVI_ADD_CODEC_SUPPORT
#define RIFF_WAVE_FORMAT_FLAC           (0xf1ac)
#endif
...
#endif
#if MTK_AVI_ADD_CODEC_SUPPORT
        case RIFF_WAVE_FORMAT_FLAC:
            LOGW("MEDIA_MIMETYPE_AUDIO_FLAC %x", id);
...
#if MTK_AVI_ADD_CODEC_SUPPORT
status_t addAVIFlacCodecInfo(const sp<MetaData> &meta, const void *_codecPrivate, size_t codecPrivateSize)
{
    int i=0;
    typedef struct {
    ...
    FLAC__StreamMetadata_Info_ flacMetadata = {0};
    
    size_t len1=0,len2=0;

    for(i=0;i<(int)codecPrivateSize-4;i++)
    {
        
        ALOGE("i=%d, buf[i+3]: 0x%x, 0x%x, 0x%x, 0x%x", i
                                                     ,*(uint8_t*)(_codecPrivate+i)
                                                     ,*(uint8_t*)(_codecPrivate+i+1)
                                                     ,*(uint8_t*)(_codecPrivate+i+2)
                                                     ,*(uint8_t*)(_codecPrivate+i+3));
        if(*(uint8_t*)(_codecPrivate+i) == 0x66 
            && *(uint8_t*)(_codecPrivate+i+1) == 0x4c 
            && *(uint8_t*)(_codecPrivate+i+2) == 0x61 
            && *(uint8_t*)(_codecPrivate+i+3) == 0x43)
        {
            break;
        }
        len1 ++;
    ...
    ALOGE("len1=%d, buf[i+4]: 0x%x", len1 ,*(uint8_t*)(_codecPrivate+len1+4));

    //bits.skipBits(32);/*start code 00 00 01 0F*/

    if((*(uint8_t*)(_codecPrivate+len1+4) == 0x80)
        || (*(uint8_t*)(_codecPrivate+len1+4) == 0x0))
    {
        len1  = len1 + 4 + 1;

        const uint8_t *codecPrivate = (const uint8_t *)(_codecPrivate+len1);
        
        len2 = (codecPrivateSize - len1)*8;
        
        ALOGE("len2=%d, codecPrivateSize=%d, len1=%d", len2 ,codecPrivateSize,len1);
        
        ABitReader bitsLen(codecPrivate, 24);
        uint32_t metaDataLen = bitsLen.getBits(24);
        ALOGE("metaDataLen=%d ", metaDataLen);
        
        ABitReader bits(codecPrivate+3, metaDataLen*8);
        
        uint32_t minBlockSize = bits.getBits(16);
        uint32_t maxBlockSize = bits.getBits(16);
        uint32_t minFrameSize = bits.getBits(24);
        uint32_t maxFrameSize = bits.getBits(24);

        uint32_t sampleRate = bits.getBits(20);
        uint32_t channelCnt = bits.getBits(3);
        uint32_t bitPerSample = bits.getBits(5);
        uint32_t totalSample = 0;//bits.getBits(36);
        

        ALOGE("metaDataLen = %d, minBlockSize = %d, maxBlockSize = %d, minFrameSize = %d, maxFrameSize = %d, sampleRate = %d, channelCnt = %d, bitPerSample = %d, totalSample = %d",
                                                                                    metaDataLen,
                                                                                    minBlockSize,
                                                                                    maxBlockSize,
                                                                                    minFrameSize,
        ...
        flacMetadata.min_blocksize = minBlockSize;
        flacMetadata.max_blocksize = maxBlockSize;
        flacMetadata.min_framesize = minFrameSize;
        flacMetadata.max_framesize = maxFrameSize;
        
        flacMetadata.sample_rate = sampleRate;
        flacMetadata.channels = channelCnt;
        flacMetadata.bits_per_sample = bitPerSample;
        flacMetadata.total_samples = totalSample;
    }
    else
    {
        ALOGE("(_codecPrivate[len1+4]) = 0x%x", *(uint8_t*)(_codecPrivate+len1+4));
    }

    uint8_t *flacMetaInfo = new uint8_t[sizeof(FLAC__StreamMetadata_Info_)];   
    memcpy(flacMetaInfo, &flacMetadata, sizeof(FLAC__StreamMetadata_Info_));
    
    meta->setData(kKeyFlacMetaInfo, 0, flacMetaInfo, sizeof(FLAC__StreamMetadata_Info_));
...
static void addESDSFromCodecPrivate(const sp<MetaData> &meta,bool isAudio, const void *priv, size_t privSize) 
{
...
static const char *BFourCC2MIME(uint32_t fourcc) {
    ...
    #if MTK_AVI_ADD_CODEC_SUPPORT
		case BFOURCC('v', 'p', '8', '0'):
			return MEDIA_MIMETYPE_VIDEO_VP8;
            
        case BFOURCC('w', 'm', 'v', '3'):
        case BFOURCC('w', 'v', 'c', '1'):
            LOGE(" MEDIA_MIMETYPE_VIDEO_WMV " FORMATFOURCC, PRINTFOURCC(fourcc));
...
private:
    ...
    status_t FindMPEGVideo(const sp<ABuffer> &sampleBuffer);
    #endif
...
status_t MtkAVIExtractor::addSample(size_t id, struct MtkAVISample *s) {
    ...
    LOGV("MtkAVIExtractor::addSample--mTracks[id]:%d ",id);
...
status_t MtkAVIExtractor::parseDataChunk(int32_t pos, int ID, int size, int sync) {
    ...
    LOGV("parseDataChunk--ID " FORMATFOURCC " at pos 0x%08lx %d", PRINTFOURCC(ID), pos, gettid());
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    LOGE("strf--end: %lld,pos: %lld,sz: %lld, %lld", end,pos,sz,end-pos);
    if ((end - pos) > sz) {
        ...
        bool valid = false;
        LOGE("strf--leftBytes: %d ", leftBytes);
    ...
    if (source->mIsVideo) {
        ...
        LOGI("size[0x%x], width[0x%x],  height[0x%x], mCompression %x",vHeader.size,vHeader.width,vHeader.height , vHeader.compression);
        ...
        if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_WMV)) {
            LOGE("Video--Codec: V_WMV-1");
            if(source->strfData != NULL)
            {
                LOGE("strfData--:strfData->size() = 0x%x ",source->strfData->size());
                LOGE("strfData--:sizeof(struct bitmapInfo) = 0x%x ",sizeof(struct bitmapInfo));
                
                uint8_t* _config = new uint8_t[sizeof(bitmapInfo) + source->strfData->size()];
        
                memcpy(_config, &vHeader, sizeof(bitmapInfo));  // copy the profile value
                memcpy(_config + sizeof(bitmapInfo), source->strfData->data(), source->strfData->size());  // reserve the first 5 BYTEs for profile, width and height indication

                meta->setData(kKeyWMVC, 0, _config, sizeof(bitmapInfo) + source->strfData->size());
                delete [] _config;
            }else{
                uint8_t* _config = new uint8_t[sizeof(bitmapInfo)];
        
                memcpy(_config, &vHeader, sizeof(bitmapInfo));  // copy the profile value
               // memcpy(_config + sizeof(bitmapInfo), source->strfData->data(), source->strfData->size());  // reserve the first 5 BYTEs for profile, width and height indication

                meta->setData(kKeyWMVC, 0, _config, sizeof(bitmapInfo));
               delete [] _config;
               LOGE("Video--Codec: V_WMV-1, no strfData, just use bitmapinfo");
            }
    	    ...
    	    #ifndef ANDROID_DEFAULT_CODE
            	LOGE("strfData--:sizeof(struct bitmapInfo) = 0x%x ",sizeof(struct bitmapInfo));
        		if (source->strfData !=NULL){//for fixed ALPS01446473
        		    LOGE("strfData--:strfData->size() = 0x%x ",source->strfData->size());
                    addESDSFromCodecPrivate(meta, false, source->strfData->data(), source->strfData->size());
                }
            #endif
        }
        ...
        #if MTK_AVI_ADD_CODEC_SUPPORT
        if(!strcmp(mime, MEDIA_MIMETYPE_AUDIO_FLAC))
        {
            //uint8_t* extraData = source->strfData->data();
			if(source->strfData->data() != NULL
				&&source->strfData->size() != 0){
			    //meta->setData(kKeyFlacMetaInfo, 0, source->strfData->data(), source->strfData->size());
			    addAVIFlacCodecInfo(meta, source->strfData->data(), source->strfData->size());
				LOGI("flac codec strfData size is %d", source->strfData->size());
        ...
        LOGI("source.mSampleSize %d, mBlockAlign %d", 
...
status_t MtkAVIExtractor::parseIDX1(MtkAVIOffT pos, MtkAVIOffT end) {
    ...
    LOGV("parseIDX1 numbers:%d", numbers);
...
status_t MtkAVIExtractor::parseFirstRIFF() {
    ...
    while(pos < oldEnd) {
        ...
        LOGV("parseFirstRIFF ID " FORMATFOURCC " at pos 0x%08lx", PRINTFOURCC(list.ID), pos);
...
static int realAVCStart(const uint8_t *start, int length)
{
	...
	for(i=0; i<length; i++)
	{
			...
			&& (start[i+2] == 0x01))
		{
			...
			{
			    LOGW("realAVCStart position %d", i);
				return i;
			}
		}
	}
    LOGW("realAVCStart not find %d", i);
	return 0;
}
...
static uint32_t FindAVCSPS(uint8_t* buf, uint32_t buflen)
{
	...
	for(;i<(int)buflen-7;i++)
	{
		...
		{
			LOGE("i=%d, buf[i+4]: 0x%x, flag:0x%x, buf[i+7]: 0x%x", i, buf[i+4],(buf[i+4] & 0x1f),buf[i+7]);
	...
	if(i==(int)buflen-7)
	{
		LOGE("can not find SPS!!, i:%d, buflen:%d " ,i ,buflen);
		return 0xff;
	}
...
#ifndef ANDROID_DEFAULT_CODE
/*this function is to find out the real MPEG data for making special data when bitmapinfo is null.
    But this is just a "idea", now havn't find this type file to check it.
*/
status_t MtkAVISource::FindMPEGVideo(const sp<ABuffer> &sampleBuffer) {
    const uint8_t *data = sampleBuffer->data();
    size_t size = sampleBuffer->size();
    ...
    while (offset + 3 < size) {
        ...
        if (currentStartCode == 0xb3 && mFormat == NULL) {
            memmove(sampleBuffer->data(), sampleBuffer->data() + offset, size - offset);
            size -= offset;
            //(void)fetchTimestamp(offset);
            offset = 0;
            sampleBuffer->setRange(0, size);
        }
        ...
        if ((prevStartCode == 0xb3 && currentStartCode != 0xb5)
            ...
            if (mFormat == NULL) {
                ...
                memmove(sampleBuffer->data(),
                        sampleBuffer->data() + offset,
                        sampleBuffer->size() - offset);

                sampleBuffer->setRange(0, sampleBuffer->size() - offset);
                size -= offset;
                //(void)fetchTimestamp(offset);
                offset = 0;
...
status_t MtkAVISource::generateCodecData(bool full) {
    if (!mIsAudio && !mIsVideo){
        LOGW("NOT V/A track,mIsAudio %d, mIsVideo %d",mIsAudio ,mIsVideo);
        return ERROR_UNSUPPORTED;
    }
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if(strfData == NULL){
            LOGE("strfData->size() is zero,get special data from mSample");
    		mIsMPEG2 = true;
            int offset = mSampleOffsets[0];
    		int size = mSampleSizes[0];
        ...
        if (!isAACADTSHeader(header)) {
			...
			realSampleRate = containerAACSampleRate == 48000? 24000: containerAACSampleRate;
...
status_t MtkAVISource::addSample(struct MtkAVISample *s) {
    if (s->size == 0 && !mIsVorbis && (!mIsVideo || mSampleSizes.size() == 0))
        return OK;
    LOGV("addSample --mSampleSizes.size(%d)", mSampleSizes.size());
