...
/*buffer size for HD */
#define DVB_FILE_BUFFER_SIZE (1024 * 768 * 4)
...
prepareBitmapBuffer()
{
    ...
    mFd = open(filepath, O_CREAT | O_RDWR | O_TRUNC, 0x777);
    ...
    mBitmapData = mmap(0, bufSize, PROT_READ | PROT_WRITE, MAP_SHARED , mFd, 0);
...
#if	DVB_SUBTITLE_SAVE_AS_YUV_FILE
status_t DVBParser::prepareYuvFile()
{
...
status_t DVBParser::parseSegment(UINT8* pui1_data,size_t size)
{
	INT32					i4_ret;
	UINT16					ui2_page_id;
	...
	//*ppt_page_inst = NULL;
	//*pfg_need_update_logical_sz = FALSE;
		...
		if (0x0F == pui1_data[ui4_idx]) /* Sync_byte is '0x0F' */
		{
			...
			else if (0x14==pui1_data[ui4_idx + 1])	/*DDS*/
			{
				...
				//	 (ui2_page_id == pt_this->t_base.t_scdb_rec.u.t_dvb_sbtl.ui2_anci_pg_id) )
				{
					...
					{
						//*pfg_need_update_logical_sz = TRUE;
					}
...
INT32 DVBParser::paintPage()
{
	...
	{	
		UINT32* 	pui4_pos = (UINT32*)((char*)mBitmapData + y * ui4RowBytes);
	...
	while(DVBR_OK == mCurrPage->getRegionInfo(ui2_rgn_idx,&t_region_info))
	{
		...
		{
#if	DVB_PAINT_BITMAPDATA_DIRECTLY
			UINT32*		pui4_pos = (UINT32*)((char*)mBitmapData + (y + t_region_info.i4_top) * ui4RowBytes + ((t_region_info.i4_left) << 2));
#endif
