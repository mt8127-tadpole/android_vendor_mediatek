...
//#define LOG_NDEBUG 0
#define LOG_TAG "RTPReceiver[WFD]"
#include <utils/Log.h>
...
void RTPReceiver::Source::queuePacket(const sp<ABuffer> &packet) {
    ...
    if (mPackets.empty()) {
		...
		if (mActiveAssembler != NULL) {
            ...
            if (err != OK) {
                ALOGE("Error, assembler returned error %d", err);
            }
        }	
...
/// @}
	...
	if(Mbit == 1)
	{
		...
		if (property_get("media.wfd-sink.latency_dbg", val, NULL)) {
			if (!strcasecmp("true", val) || !strncmp("1", val, 1)) {
				...
				debugInfo->addTimeInfoByKey(true, (rtpTime*100ll)/9, "rtprecvin", now/1000);
			}
		}
#endif
	}
	...
	#if 0
	{
		...
		if (_count<=20 /*|| _count%200==0*/) {
			ALOGI("_count=%d seqNo=%d, buffer size=%d at %s \n", _count, seqNo, buffer->size(), __FUNCTION__);
		}
		_count++;
	}
	#endif
