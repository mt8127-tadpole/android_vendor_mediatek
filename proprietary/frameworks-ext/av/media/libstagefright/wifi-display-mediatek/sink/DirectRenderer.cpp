...
//#define LOG_NDEBUG 0
#define LOG_TAG "DirectRenderer[WFD]"
#include <utils/Log.h>
...
status_t DirectRenderer::DecoderContext::init(
	...
	//ALOGI("DecoderContext::init, format='%s' \n", format->debugString().c_str());
...
void DirectRenderer::DecoderContext::queueDecoderInputBuffers() {
	...
	#if 0
	{
	   ...
	   ALOGD("[%s] into DirectRenderer::DecoderContext::queueDecoderInputBuffers", ((trackIndex == 0) ? "video" : "audio"));
	}
	#endif
