...
//#define LOG_NDEBUG 0
#define LOG_TAG "WifiDisplaySink[WFD]"
#include <utils/Log.h>
...
void WifiDisplaySink::onMediaReceiverNotify(const sp<AMessage> &msg) {
    ...
    switch (what) {
        ...
        case MediaReceiver::kWhatAccessUnit:
        {
			...
			{ // added by mtk02793
				char val[PROPERTY_VALUE_MAX]="1";
				if (property_get("media.wfd-sink.latency_dbg", val, NULL)) {
					if (!strcasecmp("true", val) || !strncmp("1", val, 1)) {
						///M: Add Latency print log{
						sp<WfdDebugInfo> debugInfo= defaultWfdDebugInfo();
						int64_t now = ALooper::GetNowUs();
						debugInfo->addTimeInfoByKey(trackIndex == 0, timeUs-220000ll, "sinkin", now/1000);
						///@}
						
					   int64_t t1 = debugInfo->getTimeInfoByKey(true, timeUs-220000ll, "rtprecvin");	
					   int64_t t2 = debugInfo->getTimeInfoByKey(true, timeUs-220000ll, "mediarecvout");
					   int64_t t3 = debugInfo->getTimeInfoByKey(true, timeUs-220000ll, "apacksourceout");
					   int64_t t4 = debugInfo->getTimeInfoByKey(true, timeUs-220000ll, "sinkin");

					   ALOGE("video rtprecv->mediarecv %lld, mediarecv->anotherpacketsource %lld, anotherpacketsource->sink %lld,  total %lld ms",
...
void WifiDisplaySink::onGetParameterRequest(
    ...
    if (strstr(content,"wfd_video_formats") != NULL) {
        ...
        if (property_get("wfdsink.sigmatest.video", val, NULL)) {
            if (!strcasecmp("true", val) || !strcmp("1", val)) {
    ...
    /*
        ...
        */
        ...
        if (property_get("wfdsink.sigmatest.audio", val, NULL)) {
            if (!strcasecmp("true", val) || !strcmp("1", val)) {
                body.append("LPCM 00000002 00, AAC 00000001 00");  // 2 ch AAC 48kHz
            } else {
                body.append(
				"LPCM 00000003 00, AAC 00000007 00");
            }
        } else {
            body.append(
				"LPCM 00000003 00, AAC 00000007 00");
