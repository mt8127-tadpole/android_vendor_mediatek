...
//#define LOG_NDEBUG 0
#define LOG_TAG "MediaReceiver[WFD]"
#include <utils/Log.h>
        ...
        case RTPReceiver::kWhatAccessUnit:
        {
            ...
            if (mMode == MODE_TRANSPORT_STREAM) {
				...
				#if 0
				{
					static int bFirst=0;
					if (bFirst<=20) {
						ALOGD("feedPacket to TSParser=%d \n", accessUnit->size());
						//bFirst=false;
					}
					bFirst++;
				}
...
void MediaReceiver::drainPackets(
	...
	//ALOGD("drainPackets(%d)=%s \n", trackIndex, format->debugString().c_str());
    ...
    while (source->hasBufferAvailable(&finalResult)) {
        ...
        if (err == OK) {
			...
			{
				static int bFirstV=true;
				static int bFirstA=true;
				if (trackIndex==0 && bFirstV) {
					ALOGD("find AU(%d) size=%d \n", trackIndex, accessUnit->size());
					bFirstV=false;
				}
				if (trackIndex==1 && bFirstA) {
					ALOGD("find AU(%d) size=%d \n", trackIndex, accessUnit->size());
					bFirstA=false;
				}
			}
