      ...
      mBuffer(NULL)     
{
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_WMV) ||
        ...
        //MPEG4, DIVX 4,5,6 //DX311
        !strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_MPEG4) ||
        //!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_MPEG1) ||
        ...
        )  // ToDo: replace it with a more clever mechanism
    {
    ...
    {
        if(DX311==mExtractor->m_eAsfCodec)
        {
            mIsMPEG4 = false;
        }
        else
...
ASFErrorType ASFSource::ReadFramePostProcess(MediaBuffer **out)
{
    ...
    while(1)
    {
        ...
        else if(DX311==mExtractor->m_eAsfCodec)
        {
            pic_type = IVOP;
        }
        else
    ...
    */
    ...
    else if(!strcasecmp(mime_present,MEDIA_MIMETYPE_VIDEO_AVC))
    {
        ...
        while(prefixLen == -1 && i++ < 1)
        {
            ...
            AVCPos = realAVCStart(buffer->data(), size);//SPS position= ptr+AVCpos

            while (ptr < end) //find pps
            {
				...
				if(true == IsFindPPS)
				{
                    ...
                    {
                        offsetStart = ptr - ((uint8_t*)buffer->data()+ AVCPos);//length of SPS and PPS
                        buffer->setRange(AVCPos, offsetStart);
                    }
					else
					{
                        ALOGE("[ASFDEBUG] ERROR Plz check PPS order");
					}
            ...
            {
                ALOGI("AVCPos = %d, ptr = 0x%x, offsetStart = %d, prefixLen = %d\n", AVCPos, ptr, offsetStart, prefixLen);
				...
				if(buffer->size())
				{
					...
					if(file_)
					{
						fwrite((void *)((uint8_t*)buffer->data()), 1, offsetStart, file_);
						fclose(file_);
					}
				}
#endif
		...
		if (i > 1) {
			ALOGI(" Error,get codec specific data ,scan > %d frames!!",i);
			bIgnoreVideo = true;
			return false;
		}
    ...
    {
        memcpy(_config, bmih->data, _config_size_extra_data);
        meta->setData(kKeyWMVC, 0, _config, _keyWMVC_size);
...
bool ASFExtractor::ParseASF()
{
    ...
    if (mAsfParser)
    {
        ...
        uint64_t numpackets = mAsfParser->asf_get_data_packets();
        ALOGI ("numpackets: %llu \n", numpackets);
        if(numpackets<=0)
        {
