...
#endif
#ifdef FEATURE_FTM_MEMINFO
    item(ITEM_MEMINFO,    uistr_meminfo),
#endif
...
static bool get_barcode_from_nvram(char *barcode_result)
{
	...
	if(j != 64)
	{
		strncpy(barcode_result, barcode_struct->barcode, 64);
	}else
	{
		strcpy(barcode_result, "unknown");
	}
#else
	LOGD("Barcode EMMC Support.\n");
	...
	while(1)
    {
		...
		else if(read_from_usb > 0)
		{
            ...
            else if(strncmp(USB_read_buffer, READ_BARCODE, strlen(READ_BARCODE))==0)
            {
                ...
                else if(strlen(result) > 128)
                {
                    strcpy(result, "fail\r\n");
                }
                else if(!strncmp(result, "unknown", strlen(result)))
                {
                ...
                #endif
                int n = write(usb_com_port, result, strlen(result));
				if(n != strlen(result))
				{
					LOGD(TAG "Write test_data in version fail\n");
				}
