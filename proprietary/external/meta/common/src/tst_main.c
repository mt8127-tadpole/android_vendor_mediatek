		...
		{	
			if(is_USB_State_PlusIn())
			{
				if(!USBFlag)
				{
					...
					g_hUsbComPort = open("/dev/ttyGS1",O_RDWR | O_NOCTTY | O_NDELAY);
					#else
                         ...
                         TST_FRMAE_INTERNAL_STRUCT *ti)
{
    ...
    while ( u16Length != input_len )
    {
		...
		{				
			...
			u16Length = u16Length + bytes_to_copy -1;
        ...
        switch ( g_cTstFrameState)
        {
			...
			case RS232_FRAME_MD_TUNNELING_CHECKSUM:
			    ...
			    { 
					...
					dumpDataInHexString(ti->frame_data_ptr-ti->frame_len,ti->frame_len ,16);
					break;
				}
