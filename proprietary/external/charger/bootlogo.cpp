...
{
    KPOC_LOGD("[ChargingAnimation: %s %d]\n",__FUNCTION__,__LINE__);
...
#else
        set_anim_version(0);
        KPOC_LOGD("[ChargingAnimation %s %d]not define ANIMATION_NEW:show old animation \n",__FUNCTION__,__LINE__); 
#endif 
    }
}
...
{
    KPOC_LOGD("[ChargingAnimation: %s %d]%d, %d",__FUNCTION__,__LINE__, capacity, cnt);

    if (get_battnotify_status())
    {
        KPOC_LOGD("[ChargingAnimation: %s %d] show_charger_error_logo, get_battnotify_status()= %d \n",__FUNCTION__,__LINE__, get_battnotify_status());
        show_charger_ov_logo();
        return;
    }
    if (showLowBattLogo)
    {
        KPOC_LOGD("[ChargingAnimation: %s %d] show_low_battery , showLowBattLogo = %d \n",__FUNCTION__,__LINE__,showLowBattLogo);
        show_low_battery();
        return;
    }
#ifdef ONLY_SHOW_CHARGING_LOGO
    if(capacity >= 99)
        show_full_battery();
    else if(capacity <=5)
        show_low_battery();
...
{
    KPOC_LOGD("[ChargingAnimation: %s %d] show  kernel logo \n",__FUNCTION__,__LINE__);
    show_kernel_logo();
}
