...
#if defined(MTK_PUMP_EXPRESS_SUPPORT)
    unsigned int ta_online  = get_int_value(CHARGER_FAST_PATH);
#endif
    KPOC_LOGI("in %s(), usb:%u ac:%u\n", __FUNCTION__,
            usb_online, ac_online);

#if defined(MTK_PUMP_EXPRESS_SUPPORT)
    return (usb_online == 1 || ac_online ==1 || ta_online == 1);
#else
    return (usb_online == 1 || ac_online ==1);
#endif
}
