...
/*
 * Show pure battery logo (static pic)
 *
 */
void show_battery(void)
{
    if (error_flag == 0)
    {
        anim_show_logo(ANIM_V0_BACKGROUND_INDEX);
    }
}
...
/*
 * Show full battery logo (static pic)
 *
 */
void show_full_battery(void)
{
    if (error_flag == 0)
    {
        anim_show_logo(FULL_BATTERY_INDEX);
    }
}
...
int get_fast_charging_state()
{
    ...
    LOG_ANIM("get_fast_charging_state: %d\n",state);
...
{ 
    ...
    if (draw_anim_mode == (DRAW_ANIM_MODE_FB)) {
        ...
        if (2 != show_animationm_ver && get_fast_charging_state()) {
            fill_animation_battery_fast_charging(capacity, (void *)fb_addr, dec_logo_addr, logo_addr, phical_screen);
        } else {
...
#ifdef MTK_PUMP_EXPRESS_SUPPORT 
            if (2 != show_animationm_ver && get_fast_charging_state()) {
                fill_animation_battery_fast_charging(capacity, (void *)outBuffer.bits, dec_logo_addr, logo_addr, phical_screen);
            } else {
