...
static void fix_stat(const char *path, struct stat *s, uint64_t *capabilities)
{
 ...
 *  <at> st: stat information of source inode
 *  <at> inum: target inode number
 *  <at> data: inode data (for special inodes e.g. symlink path etc)
 *  <at> data_len: inode data length
 *  <at> flags: source inode flags
 */
static int add_inode_with_xattr(struct stat *st, ino_t inum, 
				int flags, uint64_t capabilities)
{
	struct ubifs_ino_node *ino = node_buf;
...
static int add_dir_inode(DIR *dir, ino_t inum, loff_t size, unsigned int nlink,
			 struct stat *st, uint64_t capabilities)
{
	int fd, flags = 0;
	...
	if( inum == UBIFS_ROOT_INO || capabilities == 0)
		return add_inode(st, inum, flags);


	return add_inode_with_xattr(st, inum, flags, capabilities);
}
...
static int add_non_dir(const char *path_name, ino_t inum, unsigned int nlink,
		       unsigned char *type, struct stat *st, uint64_t capabilities)
{
	int fd, flags = 0;
	...
	if (S_ISREG(st->st_mode))
		return add_file(path_name, st, inum, flags, capabilities);
...
static int add_directory(const char *dir_name, ino_t dir_inum, struct stat *st,
			 int non_existing, uint64_t capabilities)
{
	struct dirent *entry;
	DIR *dir = NULL;
	int err = 0;
	...
	for (; !non_existing;) {
		...
		fix_stat(name, &dent_st, &capabilities);
		...
		if (S_ISDIR(dent_st.st_mode)) {
			err = add_directory(name, inum, &dent_st, 0, capabilities);
			if (err)
				goto out_free;
			nlink += 1;
			type = UBIFS_ITYPE_DIR;
		} else {
			err = add_non_dir(name, inum, 0, &type, &dent_st, capabilities);
			if (err)
				goto out_free;
		}
	...
	while (nh_elt) {
		...
		if (S_ISDIR(nh_elt->mode)) {
			err = add_directory(name, inum, &fake_st, 1, 0);
			if (err)
				goto out_free;
			nlink += 1;
			type = UBIFS_ITYPE_DIR;
		} else {
			err = add_non_dir(name, inum, 0, &type, &fake_st, 0);
			if (err)
				goto out_free;
		}
	...
	err = add_dir_inode(dir, dir_inum, size, nlink, st, capabilities);
	if (err)
		goto out_free;
...
static int write_data(void)
{
	...
	fix_stat(root, &root_st, &capabilities);

	head_flags = 0;
	err = add_directory(root, UBIFS_ROOT_INO, &root_st, !root, capabilities);
	if (err)
		return err;
