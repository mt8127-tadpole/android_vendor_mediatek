...
#if 0 //#if defined(MTK_TC1_FEATURE)
...
static BtStatus GORMcmd_HCC_Set_Local_BD_Addr(void)
{
...
#if 0 // #if defined(MTK_TC1_FEATURE)
    unsigned char Default_BDaddr_LGE_Security_Area_1[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};    
    unsigned char Default_BDaddr_LGE_Security_Area_2[6] = {0x00, 0x05, 0xc9, 0x00, 0x00, 0x00};    
    unsigned char Default_BDaddr_LGE_Security_Area_3[6] = {0x00, 0x00, 0x00, 0xc9, 0x05, 0x00};    
    unsigned char Default_BDaddr_MTK_NVRAM_1[6] = {0x00, 0x00, 0x46, 0x66, 0x20, 0x01};   
    unsigned char Default_BDaddr_MTK_NVRAM_2[6] = {0x01, 0x20, 0x66, 0x46, 0x00, 0x00};  
	...
	if ( 0 == TC1_FAC_NAME(FacReadBtAddr(MISC2_bdaddr)))
	{
		 LOG_DBG("Fail !!! Read BD address from MISC2 \r\n");		
		 return BT_STATUS_FAILED;
	}
	else
	{
			LOG_WAN("LGE_FacReadBtAddr: %02x-%02x-%02x-%02x-%02x-%02x\n",
				 MISC2_bdaddr[5], MISC2_bdaddr[4], MISC2_bdaddr[3], MISC2_bdaddr[2], MISC2_bdaddr[1], MISC2_bdaddr[0]);  
	}


	if ( (0 == memcmp(MISC2_bdaddr, Default_BDaddr_LGE_Security_Area_1, 6))
		  || (0 == memcmp(MISC2_bdaddr, Default_BDaddr_LGE_Security_Area_2, 6))
		  || (0 == memcmp(MISC2_bdaddr, Default_BDaddr_LGE_Security_Area_3, 6))
          || (0 == memcmp(btinit->bt_nvram.fields.addr, Default_BDaddr_MTK_NVRAM_1, 6))         
          || (0 == memcmp(btinit->bt_nvram.fields.addr, Default_BDaddr_MTK_NVRAM_2, 6))
		)
	{
	...
	{
		hciCommand.parms[0] = MISC2_bdaddr[5];
		hciCommand.parms[1] = MISC2_bdaddr[4];
		hciCommand.parms[2] = MISC2_bdaddr[3];
		hciCommand.parms[3] = MISC2_bdaddr[2];
		hciCommand.parms[4] = MISC2_bdaddr[1];
		hciCommand.parms[5] = MISC2_bdaddr[0];
	}
#else    
    switch(g_chipId)
    {
...
static BtStatus GORMcmd_HCC_Set_Radio(void)
{
    ...
    hciCommand.parms[1] = 0x80;
    hciCommand.parms[2] = 0x00;
    hciCommand.parms[3] = 0x06;
    hciCommand.parms[4] = 0x05;
    hciCommand.parms[5] = 0x05;
#else
