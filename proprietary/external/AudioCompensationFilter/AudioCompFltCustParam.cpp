...
//billy+++
	...
	if ( temp_buf[0] == hwA || temp_buf[0] == hwB){
                    ALOGD("[Billy] HW version == A/B, copy AUDIO_COMPFLT_AB_LID table");
		    file_lid = AP_CFG_RDCL_FILE_AUDIO_COMPFLT_AB_LID;
            } else {
                    ALOGD("[Billy] HW version >= C, copy AUDIO_COMPFLT_LID table");
