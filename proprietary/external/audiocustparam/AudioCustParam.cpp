...
int GetVolumeVer1ParamFromNV(AUDIO_VER1_CUSTOM_VOLUME_STRUCT *pPara)
{
    ...
    // get from NV ram and replace the default value
#else
//billy +++ for differ Hardware SPK
    int 	  fd;
    char 	  buffer[285], temp_buf[1];
    int           len;
    char	  hwA = 'A';
    char	  hwB = 'B';
    F_ID audio_nvram_fd;
    ...
    fd = open("/proc/img_version",O_RDONLY);
    if (fd < 0) {
	    ALOGD("[Billy] could not open /proc/img_version");
	    return 0 ;
    }
    len = read(fd,buffer,sizeof(buffer));

    if (len < 0) {
	    ALOGD("[Billy] could not read /proc/img_version");
	    close(fd);
	    return 0;
    }
    close(fd);
    memcpy(temp_buf, buffer+88, 1);
    ...
    {
	    if ( temp_buf[0] == hwA || temp_buf[0] == hwB){
		    ALOGD("[Billy] HW version == A/B, copy audio_ver1_2_custom_default table");
		    file_lid = AP_CFG_RDCL_FILE_AUDIO_VER1_2_VOLUME_CUSTOM_LID;
	    } else {
		    ALOGD("[Billy] HW version >= C, copy audio_ver1_custom_default table");
		    file_lid = AP_CFG_RDCL_FILE_AUDIO_VER1_VOLUME_CUSTOM_LID;
	    }
...
int GetANCSpeechParamFromNVRam(AUDIO_ANC_CUSTOM_PARAM_STRUCT *pSphParamAnc)
{
    ...
    F_ID anc_nvram_fd;
    int file_lid = AP_CFG_RDCL_FILE_AUDIO_ANC_LID;
    int rec_size, rec_num, result;

    result = 0;
    anc_nvram_fd = NVM_GetFileDesc(file_lid, &rec_size, &rec_num, ISREAD);
    ALOGD("+GetANCSpeechParamFromNVRam audio_nvram_fd = %d", anc_nvram_fd);
    ALOGD("GetANCSpeechParamFromNVRam, rec_size=%d, rec_num=%d", rec_size, rec_num);
    result = read(anc_nvram_fd.iFileDesc, pSphParamAnc , rec_size * rec_num);
    ALOGD("-GetANCSpeechParamFromNVRam");
    NVM_CloseFileDesc(anc_nvram_fd);
...
int SetANCSpeechParamToNVRam(AUDIO_ANC_CUSTOM_PARAM_STRUCT *pSphParamAnc)
{
    ...
    anc_nvram_fd = NVM_GetFileDesc(file_lid, &rec_size, &rec_num, ISWRITE);
    ALOGD("+SetANCSpeechParamToNVRam audio_nvram_fd = %d", anc_nvram_fd);
    result = write(anc_nvram_fd.iFileDesc, pSphParamAnc , rec_size * rec_num);
    ALOGD("-SetANCSpeechParamToNVRam");
    NVM_CloseFileDesc(anc_nvram_fd);
    return result;
}
#endif
